...
/// <summary>
/// Hub chat công khai cho khách vãng lai trên Store — cho phép anonymous (khác ManagerChatHub), định tuyến theo group-
/// theo-sessionId vì kết nối vô danh không có Context.UserIdentifier. Các method dành riêng cho nhân viên (Stage 06) tự
/// gắn [HasPermission] ở method vì class không thể để [Authorize].
/// </summary>
...
{
    /// <summary>
    /// Group nhận thông báo phiên đổi trạng thái — trang quản trị Stage 06 join group này.
    /// </summary>
    ...
    /// <summary>
    /// Nhân viên trang quản trị join để nhận SessionUpdated realtime (Stage 06 mục 6.4).
    /// </summary>
    ...
    /// <summary>
    /// Nhân viên gửi tin nhắn trực tiếp cho khách — tự nhận phiên (Ai/Waiting -> Human) ngay tại đây, không cần bước
    /// "Nhận" riêng nữa; chặn nếu phiên đang do nhân viên khác phụ trách.
    /// </summary>
    ...
    public async Task SendStaffMessage(Guid sessionId, string content, string? cardsJson = null)
    {
        ...
        await Clients.Group(sessionId.ToString())
            .SendAsync("ModeChanged", new StoreChatModeChangedPayload(StoreChatMode.Human, result.Value.StaffName));
    ...
    public async Task SendMessage(Guid sessionId, string content)
    {
        ...
        try
        {
            ...
            var aiResult = await sender.Send(
                new GenerateStoreChatAiReplyCommand(
                    sessionId,
                    content,
                    OnChunk: async delta =>
                    {
            ...
            {
                logger.LogWarning(
                    "Không tạo được phản hồi AI cho phiên {SessionId}: {Error}",
                    sessionId,
...
/// <summary>
/// Payload thống nhất cho event "ModeChanged" — StaffName null khi không phải Human.
/// </summary>
