...
/// <summary>
/// Trang quản trị phiên chat Store — tách riêng khỏi StoreChatController (công khai) để phân quyền rõ
/// ràng ở tầng route, dễ audit, đúng lý do đã tách AdminChatHistoryController khỏi ManagerChatController.
/// </summary>
[ApiVersion("1.0")]
...
{
    /// <summary>Danh sách phiên cho trang quản trị — Stage 06.</summary>
    [HttpGet("sessions")]
    ...
    /// <summary>Lịch sử đầy đủ của 1 phiên cho trang quản trị — không bị lọc bởi mốc "khách đã xoá lịch sử".</summary>
    ...
    {
        var result = await sender.Send(new GetStoreChatFullHistoryForStaffQuery(id), cancellationToken).ConfigureAwait(false);
        return HandleResult(result);
    }

    /// <summary>Nhân viên bấm "Trả lại AI" — chuyển Human -> Ai.</summary>
    ...
    public async Task<IActionResult> ReleaseAsync(Guid id, CancellationToken cancellationToken)
    {
        ...
        if (result.IsSuccess)
        {
            await hubContext.Clients.Group(id.ToString()).SendAsync(
                "ModeChanged", new StoreChatModeChangedPayload(StoreChatMode.Ai, null), cancellationToken);
    ...
    /// <summary>
    /// Xoá 1 phiên chat (kèm toàn bộ tin nhắn) — dùng dọn phiên spam/test. Xoá mềm theo quy ước
    /// BaseEntity chung của hệ thống (set DeletedAt, không xoá vật lý) — ẩn hoàn toàn khỏi trang
    /// quản trị, không có UI khôi phục.
    /// </summary>
    ...
    /// <summary>Tìm sản phẩm để gán vào tin nhắn gửi khách — cùng quyền với gửi tin (Claim).</summary>
    [HttpGet("products/search")]
    [HasPermission(Permissions.Marketing.StoreChatManagement.Claim)]
    public async Task<IActionResult> SearchProductsAsync(
        [FromQuery] string? keyword, [FromQuery] int limit, CancellationToken cancellationToken)
    {
        var result = await sender.Send(new SearchProductsForStaffQuery(keyword, limit == 0 ? 10 : limit), cancellationToken)
            .ConfigureAwait(false);
        return HandleResult(result);
    }

    /// <summary>Danh sách biến thể + màu sắc của 1 sản phẩm — hiển thị để nhân viên chọn trước khi gán vào tin nhắn.</summary>
