...
public class SupplierContractsController(IMediator mediator) : ApiController
{
    ...
    [RequiresAnyPermissions(
        ...
        Permissions.Accountant.SupplierContractManagement.Create)]
        ...
        CancellationToken cancellationToken)
    {
        ...
        return result.IsSuccess
            ? Ok(new { contractFilePath = result.Value })
            : HandleResult(result);
    }
    ...
    [HttpDelete("{id:guid}")]
    [RequiresAnyPermissions(
        Permissions.Warehouse.SupplierManagement.Delete,
        Permissions.Accountant.SupplierContractManagement.Delete)]
