    ...
    IProductDeleteService deleteService) : ControllerBase
{
    ...
    /// <summary>
    /// Upsert sản phẩm (tạo mới hoặc cập nhật nếu Id có giá trị).
    /// </summary>
    [HttpPost("upsert")]
    [ProducesResponseType(typeof(ProductDetailResponse), StatusCodes.Status200OK)]
    [ProducesResponseType(typeof(ErrorResponse), StatusCodes.Status400BadRequest)]
    ...
    {
        var (data, error) = await insertService.UpsertProductAsync(request, cancellationToken).ConfigureAwait(true);
        if (error != null)
        {
            return BadRequest(error);
        }
    ...
    {
        var error = await deleteService.RestoreProductAsync(id, cancellationToken).ConfigureAwait(true);
        if (error != null)
        {
            return NotFound(error);
        }
    ...
    {
        var error = await deleteService.RestoreProductsAsync(request, cancellationToken).ConfigureAwait(true);
        if (error != null)
        {
            return BadRequest(error);
        }
    ...
    {
        var error = await updateService.UpdateProductPriceAsync(id, request, cancellationToken).ConfigureAwait(true);
        if (error != null)
        {
            return NotFound(error);
        }
    ...
    {
        var error = await updateService.UpdateManyProductPricesAsync(request, cancellationToken).ConfigureAwait(true);
        if (error != null)
        {
            return BadRequest(error);
        }
    ...
    {
        var error = await updateService.UpdateProductStatusAsync(id, request, cancellationToken).ConfigureAwait(true);
        if (error != null)
        {
            return NotFound(error);
        }
    ...
    {
        var error = await updateService.UpdateManyProductStatusesAsync(request, cancellationToken).ConfigureAwait(true);
        if (error != null)
        {
            return BadRequest(error);
        }
