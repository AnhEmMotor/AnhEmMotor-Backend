...
namespace WebAPI.Controllers.V1
{
        ...
        IProductCategoryDeleteService deleteService) : ControllerBase
    {
        ...
        {
            var error = await updateService.UpdateAsync(id, request, cancellationToken).ConfigureAwait(true);
            if (error != null)
            {
                return NotFound(error);
            }
        ...
        {
            var error = await updateService.RestoreAsync(id, cancellationToken).ConfigureAwait(true);
            if (error != null)
            {
                return NotFound(error);
            }
        ...
        {
            var error = await updateService.RestoreManyAsync(request, cancellationToken).ConfigureAwait(true);
            if (error != null)
            {
                return BadRequest(error);
            }
