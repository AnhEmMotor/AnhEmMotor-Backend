...
#pragma warning disable CRR0035
    ...
    public async Task GetProductCategories_WithPagination_ShouldReturnCorrectData()
    {
        ...
        using (var scope = _factory.Services.CreateScope())
        {
            ...
            for (int i = 1; i <= 15; i++)
            {
                categories.Add(new ProductCategoryEntity { Name = $"Category {i}", Description = "Desc", DeletedAt = null });
            }
            for (int i = 1; i <= 5; i++)
            {
    ...
    public async Task GetProductCategories_WithFilter_ShouldReturnMatchingCategories()
    {
        ...
        using (var scope = _factory.Services.CreateScope())
        {
            ...
            await db.ProductCategories.AddRangeAsync(
                ...
                new ProductCategoryEntity { Name = "Laptop", Description = "Desc", DeletedAt = null }).ConfigureAwait(true);
            await db.SaveChangesAsync(CancellationToken.None).ConfigureAwait(true);;
        }
    ...
    public async Task GetProductCategories_WithSorting_ShouldReturnSortedCategories()
    {
        ...
        using (var scope = _factory.Services.CreateScope())
        {
            ...
            await db.ProductCategories.AddRangeAsync(
                ...
                new ProductCategoryEntity { Name = "Microsoft", Description = "Desc", DeletedAt = null }).ConfigureAwait(true);
            await db.SaveChangesAsync(CancellationToken.None).ConfigureAwait(true);;
        }
    ...
    public async Task GetDeletedProductCategories_ShouldReturnOnlyDeletedCategories()
    {
        ...
        using (var scope = _factory.Services.CreateScope())
        {
            ...
            for (int i = 1; i <= 7; i++)
            {
                categories.Add(new ProductCategoryEntity { Name = $"Deleted Category {i}", Description = "Desc", DeletedAt = DateTime.UtcNow });
            }
            for (int i = 1; i <= 10; i++)
            {
    ...
    public async Task DeleteManyProductCategories_ValidIds_ShouldDeleteAll()
    {
        ...
        // Act
        var response = await _client.PostAsJsonAsync("/api/v1/ProductCategory/delete-many", request).ConfigureAwait(true);
    ...
    public async Task DeleteManyProductCategories_WithDeletedId_ShouldNotDeleteAny()
    {
        ...
        {
            var db = scope.ServiceProvider.GetRequiredService<ApplicationDBContext>();
            var category = await db.ProductCategories.FindAsync(categoryIds[0], CancellationToken.None).ConfigureAwait(true);
            category.Should().NotBeNull();
            category!.DeletedAt.Should().BeNull();
        }
    }
    ...
    public async Task RestoreManyProductCategories_ValidIds_ShouldRestoreAll()
    {
        ...
        // Assert
        response.StatusCode.Should().Be(HttpStatusCode.OK);
        var content = await response.Content.ReadFromJsonAsync<List<ProductCategoryResponse>>(CancellationToken.None).ConfigureAwait(true);
        content.Should().NotBeNull();
