...
public class PermissionAndRole : IClassFixture<IntegrationTestWebAppFactory>, IAsyncLifetime
{
    ...
    public async Task GetAllPermissions_ReturnsFullPermissionList()
    {
        ...
        using (var scope = _factory.Services.CreateScope())
        {
            ...
            var permissionsToSeed = new[] { Warehouse.BrandManagement.View, Warehouse.ProductManagement.View };
        ...
        var content = await response!.Content
            .ReadFromJsonAsync<List<Domain.Constants.Permission.PermissionModuleMetadata>>(TestContext.Current.CancellationToken)
            .ConfigureAwait(true);
        ...
        content!.First(x => x.Id == "Permissions.Warehouse").Features.Should()
            .Contain(f => f.Permissions.Any(p => p.Id == Warehouse.BrandManagement.View));
        content!.First(x => x.Id == "Permissions.Warehouse").Features.Should()
    ...
    public async Task GetUserPermissionsById_WithViewPermission_ReturnsTargetUserPermissions()
    {
        ...
        var targetUser = await IntegrationTestAuthHelper.CreateUserWithPermissionsAsync(
            ...
            targetUsername,
            "Password123!",
            [Warehouse.ProductManagement.View, Warehouse.BrandManagement.View, Admin.FileManagement.View],
            TestContext.Current.CancellationToken)
            .ConfigureAwait(true);
    ...
    public async Task GetRolePermissions_ValidRole_ReturnsRolePermissions()
    {
        ...
        var roleId = await CreateRoleWithPermissionsInternalAsync(
            roleName,
            [Warehouse.BrandManagement.View, Warehouse.BrandManagement.Create, Warehouse.BrandManagement.Edit, Warehouse.BrandManagement.Delete, Warehouse.ProductManagement.View])
            .ConfigureAwait(true);
        var uniqueId = Guid.NewGuid().ToString("N")[..8];
        ...
        content!.Should().Contain(Warehouse.BrandManagement.View);
    ...
    public async Task CreateRole_ValidData_CreatesRoleSuccessfully()
    {
        ...
        {
            var db = scope.ServiceProvider.GetRequiredService<ApplicationDBContext>();
            await EnsurePermissionExistsAsync(db, Warehouse.BrandManagement.View).ConfigureAwait(true);
    ...
    public async Task UpdateRolePermissions_ValidData_UpdatesSuccessfully()
    {
        ...
        var roleId = await CreateRoleWithPermissionsInternalAsync(roleName, [Warehouse.BrandManagement.View, Warehouse.BrandManagement.Create])
            .ConfigureAwait(true);
        var username = $"admin_{uniqueId}";
        await IntegrationTestAuthHelper.CreateUserWithPermissionsAsync(
            _factory.Services,
        ...
        await CreateRoleWithPermissionsInternalAsync($"DummyHolder_{uniqueId}", [Warehouse.BrandManagement.View, Warehouse.BrandManagement.Create])
            .ConfigureAwait(true);
        var request = new UpdateRoleCommand { Permissions = [Warehouse.ProductManagement.View, Warehouse.ProductManagement.Create, Warehouse.ProductManagement.Edit] };
    ...
    public async Task UpdateRole_UpdateDescription_UpdatesSuccessfully()
    {
        var uniqueId = Guid.NewGuid().ToString("N")[..8];
        var roleName = $"TestRole_{uniqueId}";
        var roleId = await CreateRoleWithPermissionsInternalAsync(roleName, [Warehouse.BrandManagement.View], "Original Description")
            .ConfigureAwait(true);
        var username = $"admin_{uniqueId}";
        await IntegrationTestAuthHelper.CreateUserWithPermissionsAsync(
            _factory.Services,
    ...
    public async Task DeleteRole_ValidRole_DeletesSuccessfully()
    {
        var uniqueId = Guid.NewGuid().ToString("N")[..8];
        var roleName = $"TestRole_{uniqueId}";
        var roleId = await CreateRoleWithPermissionsInternalAsync(roleName, [Warehouse.BrandManagement.View]).ConfigureAwait(true);
        var username = $"admin_{uniqueId}";
        await IntegrationTestAuthHelper.CreateUserWithPermissionsAsync(
            _factory.Services,
            username,
    ...
    {
        var uniqueId = Guid.NewGuid().ToString("N")[..8];
        await CreateRoleWithPermissionsInternalAsync($"Role1_{uniqueId}", [Warehouse.BrandManagement.View]).ConfigureAwait(true);
        await CreateRoleWithPermissionsInternalAsync($"Role2_{uniqueId}", [Warehouse.ProductManagement.View]).ConfigureAwait(true);
        await CreateRoleWithPermissionsInternalAsync($"Role3_{uniqueId}", [Admin.FileManagement.View]).ConfigureAwait(true);
        var username = $"admin_{uniqueId}";
        await IntegrationTestAuthHelper.CreateUserWithPermissionsAsync(
            _factory.Services,
            username,
    ...
    public async Task GetHR_WithPermission_ReturnsOk()
    {
        ...
        await IntegrationTestAuthHelper.CreateUserWithPermissionsAsync(
            ...
            username,
            "Password123!",
            [Domain.Constants.Permission.Permissions.Admin.EmployeeManagement.View],
            TestContext.Current.CancellationToken)
            .ConfigureAwait(true);
        var loginResponse = await IntegrationTestAuthHelper.AuthenticateAsync(
            _client,
    ...
    public async Task ApprovePayroll_WithoutPermission_ReturnsForbidden()
    {
        ...
        await IntegrationTestAuthHelper.CreateUserWithPermissionsAsync(
            ...
            username,
            "Password123!",
            [Domain.Constants.Permission.Permissions.Admin.EmployeeManagement.View, Domain.Constants.Permission.Permissions.Accountant.PayrollManagement.View],
            TestContext.Current.CancellationToken)
            .ConfigureAwait(true);
        var loginResponse = await IntegrationTestAuthHelper.AuthenticateAsync(
            _client,
    ...
    public async Task RevokePermissionFromRole_ValidData_RemovesSuccessfully()
    {
        ...
        await CreateRoleWithPermissionsInternalAsync(
            $"OtherRole_{uniqueId}",
            [Domain.Constants.Permission.Permissions.Marketing.NewsManagement.Create])
            .ConfigureAwait(true);
        var roleId = await CreateRoleWithPermissionsInternalAsync(
            roleName,
            [Domain.Constants.Permission.Permissions.Marketing.NewsManagement.Create, Domain.Constants.Permission.Permissions.Marketing.NewsManagement.View])
            .ConfigureAwait(true);
        ...
        var loginResponse = await IntegrationTestAuthHelper.AuthenticateAsync(
            ...
            .ConfigureAwait(true);
        _client.DefaultRequestHeaders.Authorization = new AuthenticationHeaderValue("Bearer", loginResponse.AccessToken);
        var request = new UpdateRoleCommand { Permissions = [Domain.Constants.Permission.Permissions.Marketing.NewsManagement.View] };
        ...
        rolePermissions.Should()
            .NotContain(
                rp => rp.Permission!.Name
                    .Equals(Domain.Constants.Permission.Permissions.Marketing.NewsManagement.Create, StringComparison.OrdinalIgnoreCase));
    }
