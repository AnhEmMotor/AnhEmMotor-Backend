...
public class Banner : IClassFixture<IntegrationTestWebAppFactory>, IAsyncLifetime
{
    ...
    public async Task BANN_013_Create_Banner_With_Priority_And_Placement_Success()
    {
        ...
        {
            title = "Priority Test",
            image_url = "http://anh-em.com/p.jpg",
            priority = 5,
            placement = "HomeTop"
        };
        var uniqueId = Guid.NewGuid().ToString("N")[..8];
        ...
        var banner = await db.Banners
            .FirstOrDefaultAsync(
                b => string.Compare(b.Title, "Priority Test") == 0,
                TestContext.Current.CancellationToken)
            .ConfigureAwait(true);
    ...
    public async Task BANN_014_Click_Increases_ClickCount()
    {
        ...
        var response = await _client.PostAsync(
            $"/api/v1/banners/{bannerId}/click",
            null,
            TestContext.Current.CancellationToken)
            .ConfigureAwait(true);
        response!.StatusCode.Should().Be(HttpStatusCode.OK);
        var updatedBanner = await db.Banners
            .AsNoTracking()
            .FirstOrDefaultAsync(b => b.Id == bannerId, TestContext.Current.CancellationToken)
            .ConfigureAwait(true);
    ...
    public async Task BANN_020_Deactivate_Banner_Success()
    {
        ...
        var updatedBanner = await db.Banners
            .AsNoTracking()
            .FirstOrDefaultAsync(b => b.Id == banner.Id, TestContext.Current.CancellationToken)
            .ConfigureAwait(true);
