...
public class Maintenance(IntegrationTestWebAppFactory factory) : IClassFixture<IntegrationTestWebAppFactory>, IAsyncLifetime
{
    ...
    public async Task CreateVehicle_ValidLead_SavesSuccessfully()
    {
        ...
        await db.SaveChangesAsync(TestContext.Current.CancellationToken).ConfigureAwait(true);
        var savedVehicle = await db.Vehicles
            .FirstOrDefaultAsync(x => x.Id == vehicle.Id, TestContext.Current.CancellationToken)
            .ConfigureAwait(true);
    ...
    public async Task CreateMaintenanceHistory_ValidVehicle_SavesSuccessfully()
    {
        ...
        await db.SaveChangesAsync(TestContext.Current.CancellationToken).ConfigureAwait(true);
        var savedHistory = await db.MaintenanceHistories
            .FirstOrDefaultAsync(x => x.Id == history.Id, TestContext.Current.CancellationToken)
            .ConfigureAwait(true);
    ...
    public async Task CreateVehicleDocument_ValidVehicle_SavesSuccessfully()
    {
        ...
        await db.SaveChangesAsync(TestContext.Current.CancellationToken).ConfigureAwait(true);
        var savedDoc = await db.VehicleDocuments
            .FirstOrDefaultAsync(x => x.Id == doc.Id, TestContext.Current.CancellationToken)
            .ConfigureAwait(true);
    ...
    public async Task DeleteVehicle_SoftDeleteCascadesToRelatedData()
    {
        ...
        using (var scope = factory.Services.CreateScope())
        {
            ...
            var vehicle = await db.Vehicles
                ...
                .FirstOrDefaultAsync(x => x.Id == vehicleId, TestContext.Current.CancellationToken)
                .ConfigureAwait(true);
        ...
        {
            var db = scope.ServiceProvider.GetRequiredService<ApplicationDBContext>();
            var history = await db.MaintenanceHistories
                .AnyAsync(h => h.VehicleId == vehicleId, TestContext.Current.CancellationToken)
                .ConfigureAwait(true);
            var document = await db.VehicleDocuments
                .AnyAsync(d => d.VehicleId == vehicleId, TestContext.Current.CancellationToken)
                .ConfigureAwait(true);
    ...
    public async Task CreateVehicle_InvalidLeadId_ThrowsException()
    {
        using var scope = factory.Services.CreateScope();
        var db = scope.ServiceProvider.GetRequiredService<ApplicationDBContext>();
        var vehicle = new Vehicle { LeadId = 9999, VinNumber = "V6", EngineNumber = "E6", LicensePlate = "L6" };
