...
public class Quotation : IClassFixture<IntegrationTestWebAppFactory>, IAsyncLifetime
{
    ...
    public async Task CreateQuotation_Success_ReturnsCreated()
    {
        ...
        var result = await response.Content
            .ReadFromJsonAsync<QuotationDetailResponse>(TestContext.Current.CancellationToken)
            .ConfigureAwait(true);
        ...
        var dbQuotation = await assertDb.Quotations
            .FirstOrDefaultAsync(q => q.Id == result.Id, TestContext.Current.CancellationToken)
            .ConfigureAwait(true);
    ...
    public async Task CreateQuotation_NoPermission_ReturnsForbidden()
    {
        ...
        var login = await IntegrationTestAuthHelper.AuthenticateAsync(
            ...
            .ConfigureAwait(true);
        _client.DefaultRequestHeaders.Authorization = new AuthenticationHeaderValue("Bearer", login.AccessToken);
        var command = new CreateQuotationCommand { SupplierId = 1, Products = [] };
        var response = await HttpClientJsonExtensions.PostAsJsonAsync(
            _client,
            "/api/v1/Quotations",
            command,
    ...
    public async Task GetQuotationById_Success_ReturnsOk()
    {
        ...
        var db = scope.ServiceProvider.GetRequiredService<ApplicationDBContext>();
        var seeded = await SeedBaseDataAsync(db, uniqueId, TestContext.Current.CancellationToken).ConfigureAwait(true);
        var quotation = new QuotationEntity { SupplierId = seeded.Supplier.Id, Status = "draft", Note = "Test Note" };
    ...
    public async Task UpdateQuotation_Draft_Success_ReturnsOk()
    {
        ...
        var db = scope.ServiceProvider.GetRequiredService<ApplicationDBContext>();
        var seeded = await SeedBaseDataAsync(db, uniqueId, TestContext.Current.CancellationToken).ConfigureAwait(true);
        var quotation = new QuotationEntity { SupplierId = seeded.Supplier.Id, Status = "draft", Note = "Old Note" };
        db.Quotations.Add(quotation);
        await db.SaveChangesAsync(TestContext.Current.CancellationToken).ConfigureAwait(true);
        ...
        var assertDb = assertScope.ServiceProvider.GetRequiredService<ApplicationDBContext>();
        var dbQuotation = await assertDb.Quotations
            .Include(q => q.QuotationProductRows)
    ...
    public async Task SendQuotation_Success_ReturnsOk()
    {
        ...
        var db = scope.ServiceProvider.GetRequiredService<ApplicationDBContext>();
        var seeded = await SeedBaseDataAsync(db, uniqueId, TestContext.Current.CancellationToken).ConfigureAwait(true);
        var quotation = new QuotationEntity { SupplierId = seeded.Supplier.Id, Status = "draft" };
        db.Quotations.Add(quotation);
        await db.SaveChangesAsync(TestContext.Current.CancellationToken).ConfigureAwait(true);
        var response = await _client.PatchAsync(
            $"/api/v1/Quotations/{quotation.Id}/send",
            null,
            TestContext.Current.CancellationToken)
            .ConfigureAwait(true);
        response.StatusCode.Should().Be(HttpStatusCode.OK);
    ...
    public async Task ApproveQuotation_Success_ReturnsOk()
    {
        ...
        var db = scope.ServiceProvider.GetRequiredService<ApplicationDBContext>();
        var seeded = await SeedBaseDataAsync(db, uniqueId, TestContext.Current.CancellationToken).ConfigureAwait(true);
        var quotation = new QuotationEntity { SupplierId = seeded.Supplier.Id, Status = "sent" };
        db.Quotations.Add(quotation);
        await db.SaveChangesAsync(TestContext.Current.CancellationToken).ConfigureAwait(true);
    ...
    [Fact(
        DisplayName = "QUO_028 - PUT /api/v1/Quotations/{id}: Cập nhật báo giá đã duyệt thành công chỉ khi thay đổi Ghi chú")]
    public async Task UpdateQuotation_ApprovedNotes_Success()
    {
        ...
        var db = scope.ServiceProvider.GetRequiredService<ApplicationDBContext>();
        var seeded = await SeedBaseDataAsync(db, uniqueId, TestContext.Current.CancellationToken).ConfigureAwait(true);
        var quotation = new QuotationEntity { SupplierId = seeded.Supplier.Id, Status = "approved", Note = "Old Note" };
        db.Quotations.Add(quotation);
        await db.SaveChangesAsync(TestContext.Current.CancellationToken).ConfigureAwait(true);
        ...
        var command = new UpdateQuotationCommand
        {
            SupplierId = seeded.Supplier.Id,
            Notes = "New Note Only",
            Products = [new() { Id = row.Id, ProductVariantId = seeded.Variant.Id.ToString(), QuotePrice = 500 }]
        };
        var response = await HttpClientJsonExtensions.PutAsJsonAsync(
            _client,
            $"/api/v1/Quotations/{quotation.Id}",
    ...
    [Fact(
        DisplayName = "QUO_029 - PUT /api/v1/Quotations/{id}: Cập nhật báo giá đã duyệt thất bại nếu thay đổi giá hoặc sản phẩm")]
    public async Task UpdateQuotation_ApprovedModifyProducts_ReturnsBadRequest()
    {
        ...
        var command = new UpdateQuotationCommand
        {
            SupplierId = seeded.Supplier.Id,
            Notes = "New Note",
            Products = [new() { Id = row.Id, ProductVariantId = seeded.Variant.Id.ToString(), QuotePrice = 600 }]
        };
        var response = await HttpClientJsonExtensions.PutAsJsonAsync(
            _client,
            $"/api/v1/Quotations/{quotation.Id}",
    ...
    public async Task DeleteQuotation_ApprovedWithApprovePermission_ReturnsNoContent()
    {
        ...
        var db = scope.ServiceProvider.GetRequiredService<ApplicationDBContext>();
        var seeded = await SeedBaseDataAsync(db, uniqueId, TestContext.Current.CancellationToken).ConfigureAwait(true);
        var quotation = new QuotationEntity { SupplierId = seeded.Supplier.Id, Status = "approved" };
        db.Quotations.Add(quotation);
        await db.SaveChangesAsync(TestContext.Current.CancellationToken).ConfigureAwait(true);
        var response = await _client.DeleteAsync(
            $"/api/v1/Quotations/{quotation.Id}",
