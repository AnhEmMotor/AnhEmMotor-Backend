...
public class Contact : IClassFixture<IntegrationTestWebAppFactory>, IAsyncLifetime
{
    ...
    public async Task CreateReply_ValidRequest_SavesReply()
    {
        ...
        {
            var db = scope.ServiceProvider.GetRequiredService<ApplicationDBContext>();
            var c = new Domain.Entities.Contact
            {
                FullName = "Reply Target",
        ...
        await IntegrationTestAuthHelper.CreateUserWithPermissionsAsync(
            ...
            username,
            "Password123!",
            [Permissions.Marketing.CustomerManagement.Reply],
            CancellationToken.None)
            .ConfigureAwait(true);
        var loginResponse = await IntegrationTestAuthHelper.AuthenticateAsync(
            _client,
            ...
            .ConfigureAwait(true);
        _client.DefaultRequestHeaders.Authorization = new AuthenticationHeaderValue("Bearer", loginResponse.AccessToken);
        var command = new CreateContactReplyCommand
        {
            ContactId = contactId,
            Message = "Admin Reply",
        ...
        {
            var db = scope.ServiceProvider.GetRequiredService<ApplicationDBContext>();
            var contactWithReplies = await db.Contacts
                .Include(c => c.Replies)
    ...
    public async Task UpdateInternalNote_ValidRequest_UpdatesNote()
    {
        ...
        using (var scope = _factory.Services.CreateScope())
        {
            ...
            var contact = await db.Contacts
                .FirstOrDefaultAsync(x => x.Id == contactId, TestContext.Current.CancellationToken)
                .ConfigureAwait(true);
