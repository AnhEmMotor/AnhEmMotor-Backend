...
public class Brand : IClassFixture<IntegrationTestWebAppFactory>, IAsyncLifetime
{
    ...
    public async Task BRAND_052_GetBrandStatistics_Success()
    {
        ...
        using (var scope = _factory.Services.CreateScope())
        {
            ...
            await db.Database.ExecuteSqlRawAsync(
                ...
                testNow.AddMinutes(-20), testNow.AddMinutes(-10), b1.Id).ConfigureAwait(true);

            await db.Database.ExecuteSqlRawAsync(
                "UPDATE \"Brand\" SET \"CreatedAt\" = {0}, \"UpdatedAt\" = {1} WHERE \"Id\" = {2}", 
                testNow.AddMinutes(-20), testNow.AddMinutes(-5), b2.Id).ConfigureAwait(true);

            await db.Database.ExecuteSqlRawAsync(
                "UPDATE \"Brand\" SET \"CreatedAt\" = {0}, \"UpdatedAt\" = NULL WHERE \"Id\" = {1}", 
                testNow.AddMinutes(-20), b3.Id).ConfigureAwait(true);
        }
        ...
        response!.StatusCode.Should().Be(HttpStatusCode.OK);
        var stats = await response.Content.ReadFromJsonAsync<BrandStatisticsResponse>(CancellationToken.None).ConfigureAwait(true);
    ...
    public async Task BRAND_053_GetBrandStatistics_LatestTime_Comparison_Success()
    {
        ...
        using (var scope = _factory.Services.CreateScope())
        {
            ...
            var brandA = new BrandEntities 
            { 
                Name = "Brand A", 
                Origin = "Japan"
            };
            var brandB = new BrandEntities 
            { 
                Name = "Brand B", 
                Origin = "Japan"
            };
            ...
            await db.Database.ExecuteSqlRawAsync(
                "UPDATE \"Brand\" SET \"CreatedAt\" = {0}, \"UpdatedAt\" = {1} WHERE \"Id\" = {2}", 
                t1, t2, brandA.Id).ConfigureAwait(true);

            await db.Database.ExecuteSqlRawAsync(
                "UPDATE \"Brand\" SET \"CreatedAt\" = {0}, \"UpdatedAt\" = NULL WHERE \"Id\" = {1}", 
                t3, brandB.Id).ConfigureAwait(true);
        }
        ...
        using (var scope = _factory.Services.CreateScope())
        {
            ...
            await db.Database.ExecuteSqlRawAsync(
                "UPDATE \"Brand\" SET \"UpdatedAt\" = {0} WHERE \"Id\" = {1}", 
                t4, existingA.Id).ConfigureAwait(true);
        }
        ...
        var stats2 = await response2.Content.ReadFromJsonAsync<BrandStatisticsResponse>(CancellationToken.None).ConfigureAwait(true);
