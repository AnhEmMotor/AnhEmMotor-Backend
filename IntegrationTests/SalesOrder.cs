...
public class SalesOrder : IClassFixture<IntegrationTestWebAppFactory>, IAsyncLifetime
{
    ...
    public async Task GetOutputs_WithFilters_ReturnsFilteredResults()
    {
        ...
        await IntegrationTestAuthHelper.CreateUserWithPermissionsAsync(
            ...
            username,
            password,
            [Permissions.Warehouse.OutputManagement.ViewUnconfirmed],
            TestContext.Current.CancellationToken,
            email)
            .ConfigureAwait(true);
        var loginResponse = await IntegrationTestAuthHelper.AuthenticateAsync(
    ...
    public async Task DeleteOutput_ValidId_SetsDeletedAt()
    {
        ...
        var user = await IntegrationTestAuthHelper.CreateUserWithPermissionsAsync(
            ...
            username,
            password,
            [Permissions.Warehouse.OutputManagement.Delete, Permissions.Warehouse.OutputManagement.Create],
            TestContext.Current.CancellationToken,
            email)
            .ConfigureAwait(true);
        var loginResponse = await IntegrationTestAuthHelper.AuthenticateAsync(
    ...
    public async Task RestoreOutput_ValidId_ClearsDeletedAt()
    {
        ...
        var restoreResponse = await _client.PostAsync(
            $"/api/v1/SalesOrders/{order.Id}/restore",
            null,
            TestContext.Current.CancellationToken)
            .ConfigureAwait(true);
    ...
    public async Task GetDeletedOutputs_ReturnsOnlyDeletedOrders()
    {
        ...
        await IntegrationTestAuthHelper.CreateUserWithPermissionsAsync(
            ...
            username,
            password,
            [Permissions.Warehouse.OutputManagement.View],
            TestContext.Current.CancellationToken,
            email)
            .ConfigureAwait(true);
        var loginResponse = await IntegrationTestAuthHelper.AuthenticateAsync(
    ...
    public async Task UpdateOutputStatus_InvalidTransition_ReturnsBadRequest()
    {
        ...
        var user = await IntegrationTestAuthHelper.CreateUserWithPermissionsAsync(
            ...
            username,
            password,
            [Permissions.Warehouse.OutputManagement.Create, Permissions.Warehouse.OutputManagement.ChangeStatus],
            TestContext.Current.CancellationToken,
            email)
            .ConfigureAwait(true);
        var loginResponse = await IntegrationTestAuthHelper.AuthenticateAsync(
    ...
    public async Task RestoreManyOutputs_ValidIds_RestoresAllOrders()
    {
        ...
        var o1 = await r1.Content
            .ReadFromJsonAsync<OrderDetailResponse>(TestContext.Current.CancellationToken)
            .ConfigureAwait(true);
        await _client.DeleteAsync($"/api/v1/SalesOrders/{o1!.Id}", TestContext.Current.CancellationToken)
            .ConfigureAwait(true);
        var httpContent2 = new StringContent(jsonContent, Encoding.UTF8, "application/json");
        var r2 = await _client.PostAsync("/api/v1/SalesOrders", httpContent2, TestContext.Current.CancellationToken)
            .ConfigureAwait(true);
        var o2 = await r2.Content
            .ReadFromJsonAsync<OrderDetailResponse>(TestContext.Current.CancellationToken)
            .ConfigureAwait(true);
        await _client.DeleteAsync($"/api/v1/SalesOrders/{o2!.Id}", TestContext.Current.CancellationToken)
            .ConfigureAwait(true);
    ...
    public async Task GetTransitionMap_WithEditPermission_ReturnsDictionary()
    {
        ...
        await IntegrationTestAuthHelper.CreateUserWithPermissionsAsync(
            ...
            username,
            password,
            [Permissions.Warehouse.OutputManagement.Edit],
            TestContext.Current.CancellationToken,
            email)
            .ConfigureAwait(true);
        var loginResponse = await IntegrationTestAuthHelper.AuthenticateAsync(
    ...
    public async Task CreateOutput_ExceedCategoryLimit_ReturnsBadRequest()
    {
        ...
        using (var scope = _factory.Services.CreateScope())
        {
            ...
            var brand = new BrandEntity { Name = $"Brand_{uniqueId}" };
            db.Brands.Add(brand);
            var category = new ProductCategoryEntity { Name = $"LimitedCat_{uniqueId}", MaxPurchaseQuantity = 2 };
            db.ProductCategories.Add(category);
            await db.SaveChangesAsync(TestContext.Current.CancellationToken).ConfigureAwait(true);
            var product = new ProductEntity
            {
