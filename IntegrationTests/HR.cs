...
public class HR : IClassFixture<IntegrationTestWebAppFactory>, IAsyncLifetime
{
    ...
    public async Task HR01_Create_Employee_Success()
    {
        ...
        var adminLogin = await IntegrationTestAuthHelper.AuthenticateAsync(_client, "admin", "AdminPass123!", TestContext.Current.CancellationToken).ConfigureAwait(true);
        ...
        // Action
        var response = await HttpClientJsonExtensions.PostAsJsonAsync(_client, "/api/v1/hr/employees", command, TestContext.Current.CancellationToken).ConfigureAwait(true);
    ...
    public async Task HR04_Prevent_Overlapping_CommissionPolicy()
    {
        ...
        var admin = await IntegrationTestAuthHelper.CreateUserWithPermissionsAsync(
            _factory.Services, $"admin_hr04_prev_{uniqueId}", "Password123!", [], TestContext.Current.CancellationToken).ConfigureAwait(true);
        
        var login = await IntegrationTestAuthHelper.AuthenticateAsync(_client, $"admin_hr04_prev_{uniqueId}", "Password123!", TestContext.Current.CancellationToken).ConfigureAwait(true);
        ...
        await HttpClientJsonExtensions.PostAsJsonAsync(_client, "/api/v1/hr/commission-policies", command1, TestContext.Current.CancellationToken).ConfigureAwait(true);
        ...
        // Action
        var response = await HttpClientJsonExtensions.PostAsJsonAsync(_client, "/api/v1/hr/commission-policies", command2, TestContext.Current.CancellationToken).ConfigureAwait(true);
    ...
    public async Task HR04_Allow_Adjacent_CommissionPolicy()
    {
        ...
        var admin = await IntegrationTestAuthHelper.CreateUserWithPermissionsAsync(
            _factory.Services, $"admin_hr04_{uniqueId}", "Password123!", [], TestContext.Current.CancellationToken).ConfigureAwait(true);
        
        var login = await IntegrationTestAuthHelper.AuthenticateAsync(_client, $"admin_hr04_{uniqueId}", "Password123!", TestContext.Current.CancellationToken).ConfigureAwait(true);
        _client.DefaultRequestHeaders.Authorization = new AuthenticationHeaderValue("Bearer", login.AccessToken);
    ...
    public async Task HR05_Calculate_Commission_On_Order_Completion()
    {
        ...
        await HttpClientJsonExtensions.PatchAsJsonAsync(_client, $"/api/v1/SalesOrders/{orderId}/status", new UpdateOutputStatusCommand { StatusId = OrderStatus.ConfirmedCod }, TestContext.Current.CancellationToken).ConfigureAwait(true);
        await HttpClientJsonExtensions.PatchAsJsonAsync(_client, $"/api/v1/SalesOrders/{orderId}/status", new UpdateOutputStatusCommand { StatusId = OrderStatus.Delivering }, TestContext.Current.CancellationToken).ConfigureAwait(true);
        var response = await HttpClientJsonExtensions.PatchAsJsonAsync(_client, $"/api/v1/SalesOrders/{orderId}/status", new UpdateOutputStatusCommand { StatusId = OrderStatus.Completed }, TestContext.Current.CancellationToken).ConfigureAwait(true);

        response!.StatusCode.Should().Be(HttpStatusCode.OK);

        // Assert
        var commission = await db.CommissionRecords.FirstOrDefaultAsync(r => r.OutputId == orderId, TestContext.Current.CancellationToken).ConfigureAwait(true);
    ...
    public async Task HR07_Update_Payroll_Status_Paid()
    {
        ...
        var login = await IntegrationTestAuthHelper.AuthenticateAsync(_client, $"admin_hr07_{uniqueId}", "Password123!", TestContext.Current.CancellationToken).ConfigureAwait(true);
        _client.DefaultRequestHeaders.Authorization = new AuthenticationHeaderValue("Bearer", login.AccessToken);
        ...
        var user = await IntegrationTestAuthHelper.CreateUserWithPermissionsAsync(
            _factory.Services, $"user_hr07_{uniqueId}", "Password123!", [], TestContext.Current.CancellationToken).ConfigureAwait(true);
        ...
        var response = await HttpClientJsonExtensions.PostAsJsonAsync(_client, "/api/v1/hr/commissions/approve-payroll", new { month = now.Month, year = now.Year }, TestContext.Current.CancellationToken).ConfigureAwait(true);

        // Assert
        response!.StatusCode.Should().Be(HttpStatusCode.OK);
        
        var updatedRecord = await db.CommissionRecords.AsNoTracking().FirstOrDefaultAsync(r => r.Id == record.Id, TestContext.Current.CancellationToken).ConfigureAwait(true);
