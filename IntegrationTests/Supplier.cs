...
public class Supplier : IClassFixture<IntegrationTestWebAppFactory>, IAsyncLifetime
{
    ...
    [Fact(DisplayName = "SUP_041 - Xóa nhi?u Supplier th?t b?i khi m?t trong s? dó còn Input Receipt Working")]
    ...
    [Fact(DisplayName = "SUP_044 - Tính toán TotalInputValue chính xác v?i nhi?u Input Receipt")]
    public async Task GetSupplierById_MultipleInputReceipts_CalculatesCorrectTotal()
    {
        ...
        using (var scope = _factory.Services.CreateScope())
        {
            ...
            var inputInfos = new List<InputInfo>
            {
                ...
                new() { InputId = inputs[1].Id, InputPrice = 2000m, Count = 20 },
                new() { InputId = inputs[2].Id, InputPrice = 500m, Count = 30 }
            };
    ...
    [Fact(DisplayName = "SUP_045 - Tính toán TotalInputValue không bao g?m Input Receipt ? tr?ng thái khác completed")]
    public async Task GetSupplierById_MixedInputStatuses_OnlyCountsCompleted()
    {
        ...
        using (var scope = _factory.Services.CreateScope())
        {
            ...
            var inputs = new List<Input>
            {
                ...
                {
                    SupplierId = supplierId,
                    StatusId = Domain.Constants.Input.InputStatus.Cancel,
                    CreatedBy = userId
                }
            };
            await db.InputReceipts.AddRangeAsync(inputs, CancellationToken.None).ConfigureAwait(true);
            ...
            var inputInfos = new List<InputInfo>
            {
                ...
                new() { InputId = inputs[1].Id, InputPrice = 200m, Count = 50 },
                new() { InputId = inputs[2].Id, InputPrice = 300m, Count = 100 }
            };
            await db.InputInfos.AddRangeAsync(inputInfos, CancellationToken.None).ConfigureAwait(true);
            await db.SaveChangesAsync(CancellationToken.None).ConfigureAwait(true);
        }
    ...
    [Fact(DisplayName = "SUP_066 - L?y l?ch s? nh?p hàng c?a nhà cung c?p thành công")]
