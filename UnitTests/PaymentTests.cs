...
public class PaymentTests
{
    ...
    public async Task PAY_001_PayOSWebhook_Success_FullAmount()
    {
        ...
        var order = new Output
        {
            ...
            OutputInfos = new List<OutputInfo> { new OutputInfo { Price = totalAmount, Count = 1 } }
        };
        ...
        var result = await handler.Handle(new ProcessPayOSWebhookCommand(webhookData), CancellationToken.None)
            .ConfigureAwait(true);
    ...
    public async Task PAY_006_VNPayIPN_Success()
    {
        ...
        _readRepositoryMock.Setup(x => x.GetByIdAsync(orderId, It.IsAny<CancellationToken>())).ReturnsAsync(order);
        var handler = new ProcessVNPayIPNCommandHandler(
            _readRepositoryMock.Object,
            _updateRepositoryMock.Object,
            _unitOfWorkMock.Object,
            _vnpayServiceMock.Object);
        var result = await handler.Handle(new ProcessVNPayIPNCommand(query.Object), CancellationToken.None)
            .ConfigureAwait(true);
        result.IsSuccess.Should().BeTrue();
    ...
    {
        var query = new Mock<IQueryCollection>();
        var vnpayResponse = new VNPayPaymentResponse { Success = false };
        _vnpayServiceMock.Setup(x => x.PaymentExecute(It.IsAny<IQueryCollection>())).Returns(vnpayResponse);
        var handler = new ProcessVNPayIPNCommandHandler(
            _readRepositoryMock.Object,
            _updateRepositoryMock.Object,
    ...
    {
        var orderId = 203;
        var order = new Output { Id = orderId, PaymentStatus = OrderPaymentStatus.Success };
        var query = new Mock<IQueryCollection>();
        var vnpayResponse = new VNPayPaymentResponse
        {
            Success = true,
