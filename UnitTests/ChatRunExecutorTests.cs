...
public class ChatRunExecutorTests
{
    ...
    public async Task ProcessRun_SavesAccumulatedOutput_WhenCancelledMidStream()
    {
        ...
        var executor = new ChatRunExecutor(
            queue,
            provider,
            cancellationRegistry.Object,
            NullLogger<ChatRunExecutor>.Instance);
        ...
        try
        {
            ...
            finished.Should()
                .Be(cancelled.Task, "CancelAsync phải được gọi trong vòng 5s sau khi stream bị huỷ giữa chừng");
        } finally
        {
            await executor.StopAsync(TestContext.Current.CancellationToken);
        }
        savedOutputOnCancel.Should()
            .Be(
    ...
    private static async IAsyncEnumerable<SidecarEvent> StreamWithTurnBoundary()
    {
        ...
        await Task.Yield();
        yield return new SidecarEvent(ChatRunEventType.TurnBoundary, string.Empty);
        yield return new SidecarEvent(ChatRunEventType.TextDelta, "Đoạn 2 sau khi hấp thụ steering.");
        yield return new SidecarEvent("done", string.Empty);
    }

    [Fact(
        DisplayName = "EXEC_02 - Gặp turn_boundary thì chốt đoạn hiện tại thành 1 message riêng, đoạn sau tính lại từ đầu")]
    public async Task ProcessRun_SplitsSegments_OnTurnBoundary()
    {
        ...
        firstSegment.Should()
            .Be("Đoạn 1 phần a. Đoạn 1 phần b.", "đoạn trước ranh giới phải được chốt thành message riêng");
        finalSegment.Should()
            .Be("Đoạn 2 sau khi hấp thụ steering.", "đoạn sau ranh giới không được lẫn nội dung đoạn trước");
    }
    ...
    private static async IAsyncEnumerable<SidecarEvent> StreamWithMessageCorrection()
    {
        ...
        yield return new SidecarEvent(
            ChatRunEventType.MessageCorrection,
    ...
    public async Task ProcessRun_PersistsThinking_WithoutTouchingFullOutput()
    {
        ...
        appendedThinking.Should()
            .Equal(
                new[] { "{\"text\":\"Cần tra doanh thu.\"}" },
                "event thinking phải được ghi lại làm ChatRunEvent riêng để replay được");
        finalOutput.Should()
            .Be(
                "Doanh thu tháng này là 0.",
