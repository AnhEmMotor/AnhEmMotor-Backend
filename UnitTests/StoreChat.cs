...
public class StoreChatTests
{
    ...
    public async Task CreateOrRestoreSession_HasAssignedStaff_ResolvesStaffName()
    {
        ...
        var existing = new StoreChatSession { VisitorKey = "visitor-3", Mode = StoreChatMode.Human, AssignedStaffId = staffId };
        var mockReadRepo = new Mock<IStoreChatReadRepository>();
        mockReadRepo.Setup(x => x.GetSessionByVisitorKeyAsync("visitor-3", It.IsAny<CancellationToken>()))
            .ReturnsAsync(existing);
        mockReadRepo.Setup(x => x.GetStaffNameAsync(staffId, It.IsAny<CancellationToken>())).ReturnsAsync("Nguyễn Văn B");

        var handler = new CreateOrRestoreStoreChatSessionCommandHandler(
            mockReadRepo.Object, new Mock<IStoreChatInsertRepository>().Object, new Mock<IUnitOfWork>().Object);
        var result = await handler.Handle(new CreateOrRestoreStoreChatSessionCommand("visitor-3"), CancellationToken.None);
    ...
    public async Task CreateOrRestoreSession_WithPreviousSessionId_LinksAndCopiesContactInfo()
    {
        ...
        {
            Id = previousId, VisitorKey = "old-visitor", ContactName = "Nguyễn Văn A", ContactPhone = "0901234567"
        };
        var mockReadRepo = new Mock<IStoreChatReadRepository>();
        var mockInsertRepo = new Mock<IStoreChatInsertRepository>();
        var mockUnitOfWork = new Mock<IUnitOfWork>();
        ...
        var result = await handler.Handle(
            new CreateOrRestoreStoreChatSessionCommand("new-visitor", previousId), CancellationToken.None);

        Assert.True(result.IsSuccess);
        Assert.Equal("Nguyễn Văn A", result.Value.ContactName);
        Assert.Equal("0901234567", result.Value.ContactPhone);
        mockInsertRepo.Verify(
            x => x.AddSession(It.Is<StoreChatSession>(s => s.PreviousSessionId == previousId)), Times.Once);
    }
    ...
    public async Task CreateOrRestoreSession_PreviousSessionIdNotFound_StillCreatesSessionWithoutLink()
    {
        ...
        var handler = new CreateOrRestoreStoreChatSessionCommandHandler(
            mockReadRepo.Object, mockInsertRepo.Object, mockUnitOfWork.Object);
        var result = await handler.Handle(
            new CreateOrRestoreStoreChatSessionCommand("new-visitor-2", Guid.NewGuid()), CancellationToken.None);
    ...
    public async Task CreateOrRestoreSession_VisitorKeyBelongsToSoftDeletedSession_ReleasesKeyBeforeCreating()
    {
        ...
        var handler = new CreateOrRestoreStoreChatSessionCommandHandler(
            mockReadRepo.Object, mockInsertRepo.Object, mockUnitOfWork.Object);
        var result = await handler.Handle(new CreateOrRestoreStoreChatSessionCommand("visitor-4"), CancellationToken.None);
        ...
        mockInsertRepo.Verify(
            x => x.AddSession(It.Is<StoreChatSession>(s => s.VisitorKey == "visitor-4")), Times.Once);
    }
    ...
    public async Task SetContactInfo_SessionExists_UpdatesNameAndPhone()
    {
        ...
        var handler = new SetStoreChatContactInfoCommandHandler(mockReadRepo.Object, mockUpdateRepo.Object, mockUnitOfWork.Object);
        var result = await handler.Handle(new SetStoreChatContactInfoCommand(sessionId, "Nguyễn Văn A", "0901234567"), CancellationToken.None);
    ...
    public async Task SetContactInfo_SessionMissing_ReturnsNotFound()
    {
        ...
        var handler = new SetStoreChatContactInfoCommandHandler(
            mockReadRepo.Object, new Mock<IStoreChatUpdateRepository>().Object, new Mock<IUnitOfWork>().Object);
        var result = await handler.Handle(new SetStoreChatContactInfoCommand(Guid.NewGuid(), "A", "0901234567"), CancellationToken.None);
    ...
    public async Task DeleteSession_SessionExists_DeletesAndSaves()
    {
        ...
        var handler = new DeleteStoreChatSessionCommandHandler(mockReadRepo.Object, mockDeleteRepo.Object, mockUnitOfWork.Object);
    ...
    public async Task DeleteSession_SessionMissing_ReturnsNotFound()
    {
        ...
        var handler = new DeleteStoreChatSessionCommandHandler(
            mockReadRepo.Object, new Mock<IStoreChatDeleteRepository>().Object, new Mock<IUnitOfWork>().Object);
    ...
    public async Task GenerateAiReply_SessionExists_PersistsCardsJsonFromAiClient()
    {
        ...
        mockUnitOfWork.Verify(x => x.SaveChangesAsync(It.IsAny<CancellationToken>()), Times.Once);
        // Session được load TRƯỚC khi gọi AI — nếu AI gọi tool escalate_to_staff giữa chừng (đổi Mode ở
        // DbContext khác), UpdateSession(session) sẽ ghi đè Mode cũ trong bộ nhớ lên giá trị mới vừa
        // set. Phải dùng TouchLastMessageAtAsync (ExecuteUpdateAsync chỉ đúng 1 cột) thay vì UpdateSession.
        mockUpdateRepo.Verify(x => x.UpdateSession(It.IsAny<StoreChatSession>()), Times.Never);
        mockUpdateRepo.Verify(x => x.TouchLastMessageAtAsync(sessionId, It.IsAny<DateTime>(), It.IsAny<CancellationToken>()), Times.Once);
    }
    ...
    {
        // AI vừa escalate_to_staff xong dừng lại luôn (route_after_tools bên AISidecar), không sinh
        // thêm text — hệ thống đã tự thông báo bằng tin nhắn System riêng, không cần tạo thêm 1 tin
        // nhắn Ai rỗng vô nghĩa ở đây (tránh 1 bong bóng trống hiện lên sau dòng thông báo System).
        var sessionId = Guid.NewGuid();
        var session = new StoreChatSession { Id = sessionId };
        var mockReadRepo = new Mock<IStoreChatReadRepository>();
        var mockInsertRepo = new Mock<IStoreChatInsertRepository>();
        ...
        mockAiClient
            .Setup(x => x.GetReplyAsync(sessionId, "Cho tôi gặp nhân viên", It.IsAny<IReadOnlyList<StoreChatHistoryItem>>(), It.IsAny<CancellationToken>()))
            .ReturnsAsync(new StoreChatAiReplyResult("", null));

        var handler = new GenerateStoreChatAiReplyCommandHandler(
            mockReadRepo.Object, mockInsertRepo.Object, mockUpdateRepo.Object, mockAiClient.Object, mockUnitOfWork.Object);
        var result = await handler.Handle(new GenerateStoreChatAiReplyCommand(sessionId, "Cho tôi gặp nhân viên"), CancellationToken.None);
    ...
    public async Task RequestHandoff_FromAi_TransitionsToWaiting()
    {
        ...
        var handler = new RequestHandoffCommandHandler(
            mockReadRepo.Object, mockUpdateRepo.Object, mockInsertRepo.Object, mockUnitOfWork.Object,
            new Mock<Microsoft.Extensions.Logging.ILogger<RequestHandoffCommandHandler>>().Object);
        var result = await handler.Handle(new RequestHandoffCommand(sessionId, "Nguyễn Văn A", "0901234567", "Customer"), CancellationToken.None);
        ...
        Assert.Equal("0901234567", session.ContactPhone);
        mockUpdateRepo.Verify(x => x.UpdateSession(session), Times.Once);
        // TriggeredBy="Customer" (khách tự bấm nút) — không tạo tin nhắn System, khác với AI tự escalate.
    ...
    public async Task RequestHandoff_TriggeredByAi_InsertsSystemMessageAndReturnsIt()
    {
        ...
        var handler = new RequestHandoffCommandHandler(
            mockReadRepo.Object, new Mock<IStoreChatUpdateRepository>().Object, mockInsertRepo.Object,
            new Mock<IUnitOfWork>().Object,
            new Mock<Microsoft.Extensions.Logging.ILogger<RequestHandoffCommandHandler>>().Object);
        var result = await handler.Handle(new RequestHandoffCommand(sessionId, null, null, "Ai"), CancellationToken.None);
        ...
        mockInsertRepo.Verify(
            x => x.AddMessage(It.Is<StoreChatMessage>(m => m.Sender == StoreChatSender.System && m.SessionId == sessionId)),
            Times.Once);
    }
    ...
    public async Task RequestHandoff_AlreadyHuman_DoesNotDowngradeMode()
    {
        ...
        var handler = new RequestHandoffCommandHandler(
            mockReadRepo.Object, new Mock<IStoreChatUpdateRepository>().Object,
            new Mock<IStoreChatInsertRepository>().Object, new Mock<IUnitOfWork>().Object,
            new Mock<Microsoft.Extensions.Logging.ILogger<RequestHandoffCommandHandler>>().Object);
        var result = await handler.Handle(new RequestHandoffCommand(sessionId, null, null, "Customer"), CancellationToken.None);
    ...
    {
        var mockUpdateRepo = new Mock<IStoreChatUpdateRepository>();
        mockUpdateRepo.Setup(x => x.TryReleaseAsync(It.IsAny<Guid>(), It.IsAny<CancellationToken>())).ReturnsAsync(false);

        var handler = new ReleaseStoreChatSessionCommandHandler(
            mockUpdateRepo.Object, new Mock<Microsoft.Extensions.Logging.ILogger<ReleaseStoreChatSessionCommandHandler>>().Object);
    ...
    {
        var waiting = new StoreChatSessionListItemDto { Id = Guid.NewGuid(), Mode = StoreChatMode.Waiting, LastMessageAt = DateTime.UtcNow };
        var human = new StoreChatSessionListItemDto { Id = Guid.NewGuid(), Mode = StoreChatMode.Human, LastMessageAt = DateTime.UtcNow };
        var ai = new StoreChatSessionListItemDto { Id = Guid.NewGuid(), Mode = StoreChatMode.Ai, LastMessageAt = DateTime.UtcNow };
        var mockReadRepo = new Mock<IStoreChatReadRepository>();
    ...
    public async Task StaffSendMessage_FromAiOrWaiting_AutoAssignsAndPersistsAsStaffSender(string initialMode)
    {
        ...
        mockUpdateRepo.Setup(x => x.TryAssignStaffAsync(sessionId, staffId, It.IsAny<CancellationToken>())).ReturnsAsync(true);
        ...
        var handler = new SendStoreChatStaffMessageCommandHandler(
            mockReadRepo.Object, mockInsertRepo.Object, mockUpdateRepo.Object, mockCurrentUserContext.Object, mockUnitOfWork.Object);
        var result = await handler.Handle(new SendStoreChatStaffMessageCommand(sessionId, "Dạ shop nghe ạ"), CancellationToken.None);

        Assert.True(result.IsSuccess);
        Assert.Equal(StoreChatSender.Staff, result.Value.Message.Sender);
        Assert.Equal("Trần Thị C", result.Value.StaffName);
        mockInsertRepo.Verify(x => x.AddMessage(It.Is<StoreChatMessage>(m => m.Sender == StoreChatSender.Staff)), Times.Once);
    }
    ...
    public async Task StaffSendMessage_WithCardsJson_PersistsCardsJsonOnMessage()
    {
        ...
        var handler = new SendStoreChatStaffMessageCommandHandler(
            mockReadRepo.Object, mockInsertRepo.Object, mockUpdateRepo.Object, mockCurrentUserContext.Object, mockUnitOfWork.Object);
        var result = await handler.Handle(
            new SendStoreChatStaffMessageCommand(sessionId, "Đây là mẫu xe anh hỏi ạ", cardsJson), CancellationToken.None);
    ...
    public async Task StaffSendMessage_EmptyContentWithCardsJson_StillSucceeds()
    {
        ...
        var handler = new SendStoreChatStaffMessageCommandHandler(
            mockReadRepo.Object, mockInsertRepo.Object, mockUpdateRepo.Object, mockCurrentUserContext.Object, new Mock<IUnitOfWork>().Object);
        var result = await handler.Handle(new SendStoreChatStaffMessageCommand(sessionId, "", cardsJson), CancellationToken.None);
    ...
    {
        var sessionId = Guid.NewGuid();
        var session = new StoreChatSession { Id = sessionId, Mode = StoreChatMode.Human, AssignedStaffId = Guid.NewGuid() };
        var mockReadRepo = new Mock<IStoreChatReadRepository>();
        var mockCurrentUserContext = new Mock<ICurrentUserContext>();
        mockReadRepo.Setup(x => x.GetSessionByIdAsync(sessionId, It.IsAny<CancellationToken>())).ReturnsAsync(session);
        ...
        var handler = new SendStoreChatStaffMessageCommandHandler(
            ...
            new Mock<IUnitOfWork>().Object);
        var result = await handler.Handle(new SendStoreChatStaffMessageCommand(sessionId, "Xin chào"), CancellationToken.None);
    ...
    public async Task StaffSendMessage_LostRaceToAnotherStaff_ReturnsConflict()
    {
        ...
        mockReadRepo.Setup(x => x.GetSessionByIdAsync(sessionId, It.IsAny<CancellationToken>())).ReturnsAsync(session);
        mockCurrentUserContext.Setup(x => x.GetUserId()).Returns(staffId);
        mockUpdateRepo.Setup(x => x.TryAssignStaffAsync(sessionId, staffId, It.IsAny<CancellationToken>())).ReturnsAsync(false);
    ...
    public async Task StaffSendMessage_SessionMissing_ReturnsNotFound()
    {
        ...
        var handler = new SendStoreChatStaffMessageCommandHandler(
            ...
            new Mock<IUnitOfWork>().Object);
        var result = await handler.Handle(new SendStoreChatStaffMessageCommand(Guid.NewGuid(), "Hi"), CancellationToken.None);
    ...
    public async Task SearchProductsForStaff_ReturnsMappedItems()
    {
        var product = new Domain.Entities.Product
        {
            ...
            ProductVariants =
            [
                new Domain.Entities.ProductVariant { Id = 1, Price = 90000000, CoverImageUrl = "sh.jpg" },
                new Domain.Entities.ProductVariant { Id = 2, Price = 95000000 }
            ]
        };
        var mockProductRepo = new Mock<IProductReadRepository>();
        mockProductRepo.Setup(r => r.GetPagedProductsAsync(
                "SH", It.IsAny<List<string>>(), It.IsAny<List<int>>(), It.IsAny<List<int>>(), It.IsAny<List<int>>(),
                null, null, 1, 10, null, null, It.IsAny<CancellationToken>()))
            .ReturnsAsync(([product], 1, new List<Application.Common.Models.FilterGroup>()));
        ...
        var product = new Domain.Entities.Product
        {
            ...
            [
                new Domain.Entities.ProductVariant
                {
                    ...
                    ProductVariantColors = [new Domain.Entities.ProductVariantColor { Id = 9, ColorName = "Đỏ đen", ColorCode = "#c00", CoverImageUrl = "red.jpg" }]
                }
            ]
        };
        var mockProductRepo = new Mock<IProductReadRepository>();
        mockProductRepo.Setup(r => r.GetByIdWithDetailsAsync(42, It.IsAny<CancellationToken>(), It.IsAny<Domain.Constants.DataFetchMode>()))
    ...
    {
        var mockProductRepo = new Mock<IProductReadRepository>();
        mockProductRepo.Setup(r => r.GetByIdWithDetailsAsync(It.IsAny<int>(), It.IsAny<CancellationToken>(), It.IsAny<Domain.Constants.DataFetchMode>()))
