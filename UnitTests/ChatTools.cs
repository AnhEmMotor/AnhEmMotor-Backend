...
public class ChatTools
{
    ...
    public async Task Search_ValidKeyword_ReturnsMappedProducts()
    {
        var product = new DomainProduct
        {
            ...
            ProductVariants =
                [new DomainProductVariant { Id = 10, Price = 20000000 }, new DomainProductVariant
                {
                    Id = 11,
                    Price = 22000000
                }]
    ...
    [Fact(
        DisplayName = "CHATTOOLS_110 - Unit - SearchProductsForChatQueryHandler tìm được sản phẩm khi search LIKE thường không ra kết quả do lệch dấu tiếng Việt")]
    public async Task Search_KeywordCoDauNhungTenSanPhamThieuDau_VanTimDuocQuaFallback()
    {
        ...
        var result = await handler.Handle(
            new SearchProductsForChatQuery { Keyword = "Nhông sên đĩa" },
    ...
    [Fact(
        DisplayName = "CHATTOOLS_111 - Unit - SearchProductsForChatQueryHandler tìm được sản phẩm khi tên sản phẩm thiếu dấu hoàn toàn")]
    public async Task Search_KeywordCoDauNhungTenSanPhamKhongCoDau_VanTimDuocQuaFallback()
    {
        ...
        var handler = new SearchProductsForChatQueryHandler(_productReadRepositoryMock.Object, _dateProvider);
        var result = await handler.Handle(
            new SearchProductsForChatQuery { Keyword = "Phuộc nhún" },
            CancellationToken.None)
            .ConfigureAwait(true);
        result.IsSuccess.Should().BeTrue();
        result.Value!.Items.Should().ContainSingle();
    ...
    [Fact(
        DisplayName = "CHATTOOLS_112 - Unit - SearchProductsForChatQueryHandler tìm được sản phẩm khi lệch vị trí dấu tiếng Việt")]
    public async Task Search_KeywordLechViTriDau_VanTimDuocQuaFallback()
    {
        ...
        var handler = new SearchProductsForChatQueryHandler(_productReadRepositoryMock.Object, _dateProvider);
        var result = await handler.Handle(
            new SearchProductsForChatQuery { Keyword = "vành đúc" },
            CancellationToken.None)
            .ConfigureAwait(true);
        result.IsSuccess.Should().BeTrue();
        result.Value!.Items.Should().ContainSingle();
    ...
    [Fact(
        DisplayName = "CHATTOOLS_113 - Unit - SearchProductsForChatQueryHandler tìm được sản phẩm khi từ khoá không liên tục trong tên (vd \"sh 2024\" vs \"Honda SH 150i 2024\")")]
    public async Task Search_KeywordKhongLienTucTrongTen_VanTimDuocQuaFallback()
    {
        ...
        var handler = new SearchProductsForChatQueryHandler(_productReadRepositoryMock.Object, _dateProvider);
        var result = await handler.Handle(
            new SearchProductsForChatQuery { Keyword = "sh 2024" },
            CancellationToken.None)
            .ConfigureAwait(true);
        result.IsSuccess.Should().BeTrue();
        result.Value!.Items.Should().ContainSingle();
    ...
    [Fact(
        DisplayName = "CHATTOOLS_114 - Unit - SearchProductsForChatQueryHandler KHÔNG khớp nếu chỉ 1 trong các từ khoá xuất hiện")]
    public async Task Search_ChiMotTuKhopTrongTen_KhongDuocTinhLaKhop()
    {
        ...
        var handler = new SearchProductsForChatQueryHandler(_productReadRepositoryMock.Object, _dateProvider);
        var result = await handler.Handle(
            new SearchProductsForChatQuery { Keyword = "sh 2025" },
            CancellationToken.None)
            .ConfigureAwait(true);
        result.IsSuccess.Should().BeTrue();
    ...
    public async Task GetProductStock_ValidProduct_ReturnsVariantStock()
    {
        var product = new DomainProduct
        {
            Id = 1,
            Name = "Vision",
            ProductVariants = [new DomainProductVariant { Id = 10, Price = 30000000 }]
        };
        _productReadRepositoryMock.Setup(
            r => r.GetByIdWithVariantsAsync(
                It.IsAny<IEnumerable<int>>(),
                It.IsAny<CancellationToken>(),
                It.IsAny<DataFetchMode>()))
            .ReturnsAsync([product]);
        _statisticalReadRepositoryMock.Setup(r => r.GetProductStockAndPriceAsync(10, It.IsAny<bool>(), It.IsAny<CancellationToken>()))
            .ReturnsAsync(new ProductStockPriceResponse { UnitPrice = 30000000, StockQuantity = 15 });
        var handler = new GetProductStockForChatQueryHandler(
            _productReadRepositoryMock.Object,
            _statisticalReadRepositoryMock.Object,
            _dateProvider);
    ...
    public async Task GetProductDetail_ValidProduct_MapsVariantSlug()
    {
        ...
        _productReadRepositoryMock.Setup(
            r => r.GetByIdWithDetailsAsync(1, It.IsAny<CancellationToken>(), It.IsAny<DataFetchMode>()))
            .ReturnsAsync(product);
    ...
    [Fact(
        DisplayName = "CHATTOOLS_114 - Unit - GetProductDetailForChatQueryHandler map đúng danh sách màu theo biến thể")]
                        ...
                        [new ProductVariantColor
                            {
                                ...
                                ColorName = "Đỏ",
                                ColorCode = "#FF0000",
                                CoverImageUrl = "do.jpg"
                            }, new ProductVariantColor
                            {
                                Id = 2,
                                ColorName = "Đen",
                                ColorCode = "#000000",
                                CoverImageUrl = "den.jpg"
                            }, ]
                }]
        };
        _productReadRepositoryMock.Setup(
    ...
    public async Task GetLowStock_FiltersOutInStockProducts()
    {
        ...
        _statisticalReadRepositoryMock.Setup(
            r => r.GetProductPerformanceTableAsync(
        ...
        var result = await handler.Handle(new GetLowStockProductsForChatQuery(), CancellationToken.None)
            .ConfigureAwait(true);
        result.IsSuccess.Should().BeTrue();
    ...
    public async Task GetOrderStatus_ValidOrder_ReturnsStatus()
    {
        ...
        _outputReadRepositoryMock.Setup(
            r => r.GetByIdWithDetailsAsync(123, It.IsAny<CancellationToken>(), It.IsAny<DataFetchMode>()))
            .ReturnsAsync(order);
        var handler = new GetOrderStatusForChatQueryHandler(_outputReadRepositoryMock.Object, _dateProvider);
        var result = await handler.Handle(
            new GetOrderStatusForChatQuery { Keyword = "Nguyễn Văn A" },
            CancellationToken.None)
            .ConfigureAwait(true);
        result.IsSuccess.Should().BeTrue();
        result.Value!.Items.Should().ContainSingle();
    ...
    public async Task GetOrderStatus_NoMatch_ReturnsEmptyItems()
    {
        ...
        var handler = new GetOrderStatusForChatQueryHandler(_outputReadRepositoryMock.Object, _dateProvider);
        var result = await handler.Handle(
            new GetOrderStatusForChatQuery { Keyword = "Không tồn tại" },
            CancellationToken.None)
            .ConfigureAwait(true);
        result.IsSuccess.Should().BeTrue();
        result.Value!.Items.Should().BeEmpty();
    ...
    public async Task GetSalesSummary_LimitsRecordsReturned()
    {
        var daily = Enumerable.Range(0, 15)
            .Select(
                i => new DailyRevenueResponse
                {
                    ReportDay = DateOnly.FromDateTime(DateTime.Now.AddDays(-i)),
                    TotalRevenue = i * 1000000
                })
            .ToList();
        _statisticalReadRepositoryMock.Setup(
            r => r.GetDailyRevenueAsync(
                It.IsAny<DateTimeOffset>(),
                It.IsAny<DateTimeOffset>(),
                It.IsAny<bool>(), It.IsAny<CancellationToken>()))
            .ReturnsAsync(daily);
    ...
    [Fact(
        DisplayName = "CHATTOOLS_110 - Unit - GetSalesSummaryForChatQueryHandler dùng đúng FromDate/ToDate được truyền vào")]
    public async Task GetSalesSummary_UsesExactFromDateToDate()
    {
        ...
        _statisticalReadRepositoryMock.Setup(
            r => r.GetDailyRevenueAsync(
                ...
                It.IsAny<DateTimeOffset>(),
                It.IsAny<bool>(), It.IsAny<CancellationToken>()))
            .Callback<DateTimeOffset, DateTimeOffset, CancellationToken>(
                (s, e, _) =>
                {
                    capturedStart = s;
                    capturedEnd = e;
        ...
        await handler.Handle(
            new GetSalesSummaryForChatQuery { FromDate = fromDate, ToDate = toDate },
            CancellationToken.None)
            .ConfigureAwait(true);
    ...
    {
        var topProducts = new List<TopProductRevenueResponse>
        {
            new() { ProductName = "Vision", UnitsSold = 20, Revenue = 600000000 }
        };
        _statisticalReadRepositoryMock.Setup(
