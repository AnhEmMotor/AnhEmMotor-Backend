...
public class News
{
    ...
    public async Task CreateNews_NoSlugProvided_GeneratesSlugFromTitle()
    {
        ...
        var handler = new CreateNewsCommandHandler(
            _newsInsertRepoMock.Object,
            _newsReadRepoMock.Object,
            _unitOfWorkMock.Object);
        await handler.Handle(command, CancellationToken.None).ConfigureAwait(true);
        _newsInsertRepoMock.Verify(
            x => x.Add(It.Is<Domain.Entities.News>(n => string.Compare(n.Slug, "tin-tuc-moi") == 0)),
            Times.Once);
    }
    ...
    public async Task CreateNews_CustomSlugProvided_UsesCustomSlug()
    {
        ...
        await handler.Handle(command, CancellationToken.None).ConfigureAwait(true);
        _newsInsertRepoMock.Verify(
            x => x.Add(It.Is<Domain.Entities.News>(n => string.Compare(n.Slug, "slug-tuy-chinh") == 0)),
            Times.Once);
    }
    ...
    [Fact(DisplayName = "NEWS_016 - Kiểm tra ràng buộc độ dài tiêu đề SEO")]
    public void CreateNewsCommandValidator_MetaTitleTooLong_ShouldHaveValidationError()
    {
