...
public class InventoryReceiptsSupplierHistory
{
    ...
    public async Task GetInventoryReceiptsBySupplierId_SupplierExists_ReturnsPagedList()
    {
        ...
        var query = new GetInventoryReceiptsBySupplierIdQuery { SupplierId = 1, SieveModel = new Sieve.Models.SieveModel { Page = 1, PageSize = 10 } };
        var supplier = new Domain.Entities.Supplier { Id = 1, Name = "Supplier A" };
        var paged = new PagedResult<InventoryReceiptListResponse>(new List<InventoryReceiptListResponse> { new() { Id = 1, SupplierId = 1 } }, 1, 1, 10);

        supplierRepoMock.Setup(x => x.GetByIdAsync(1, It.IsAny<CancellationToken>(), It.IsAny<DataFetchMode>()))
            .ReturnsAsync(supplier);
        receiptRepoMock.Setup(x => x.GetPagedAsync<InventoryReceiptListResponse>(
                It.IsAny<Sieve.Models.SieveModel>(),
                It.IsAny<DataFetchMode>(),
                It.IsAny<System.Linq.Expressions.Expression<System.Func<Domain.Entities.InventoryReceipt, bool>>>(),
    ...
    [Fact(DisplayName = "IR_051 - GetInventoryReceiptsBySupplierId returns empty list when supplier exists but has no receipts")]
    ...
    public async Task GetInventoryReceiptsBySupplierId_SupplierNotFound_ReturnsNotFound()
    {
        var supplierRepoMock = new Mock<ISupplierReadRepository>();
        var receiptRepoMock = new Mock<IInventoryReceiptReadRepository>();
        var query = new GetInventoryReceiptsBySupplierIdQuery { SupplierId = 999, SieveModel = new Sieve.Models.SieveModel { Page = 1, PageSize = 10 } };
