...
public class GetWarrantyTermByIdQueryHandler(
	IWarrantyTermReadRepository readRepository) : IRequestHandler<GetWarrantyTermByIdQuery, Result<WarrantyTermResponse?>>
{
	public async Task<Result<WarrantyTermResponse?>> Handle(
		GetWarrantyTermByIdQuery request,
		CancellationToken cancellationToken)
	{
		var term = await readRepository.GetByIdAsync(request.Id, cancellationToken, include: q => q.Include(t => t.Brand)).ConfigureAwait(false);
		if (term == null)
		{
