...
public class GetWarrantyTermsListQueryHandler(
	IWarrantyTermReadRepository readRepository) : IRequestHandler<GetWarrantyTermsListQuery, Result<PagedResult<WarrantyTermResponse>>>
{
	public async Task<Result<PagedResult<WarrantyTermResponse>>> Handle(
		GetWarrantyTermsListQuery request,
		CancellationToken cancellationToken)
	{
		var terms = await readRepository.GetAllAsync(cancellationToken, q => q.Include(t => t.Brand)).ConfigureAwait(false);
		...
		{
			var filterParts = request.SieveModel.Filters.Split(',', StringSplitOptions.RemoveEmptyEntries | StringSplitOptions.TrimEntries);
		...
		{
			var sortParts = request.SieveModel.Sorts.Split(',', StringSplitOptions.RemoveEmptyEntries | StringSplitOptions.TrimEntries);
			foreach (var part in sortParts)
			{
				...
				query = field switch
				{
					...
					"durationmonths" => isDesc ? query.OrderByDescending(t => t.DurationMonths) : query.OrderBy(t => t.DurationMonths),
					"effectivedate" => isDesc ? query.OrderByDescending(t => t.EffectiveDate) : query.OrderBy(t => t.EffectiveDate),
					_ => query
				};
			}
		}
		...
		var responses = pagedResult.Select(t => new WarrantyTermResponse
		{
			Id = t.Id,
			BrandId = t.BrandId,
