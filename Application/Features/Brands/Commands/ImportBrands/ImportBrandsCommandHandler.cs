...
{
    public async Task<Result<ImportBrandsResult>> Handle(ImportBrandsCommand request, CancellationToken cancellationToken)
    {
        ...
        for (int i = 5; i <= rowCount; i++)
        {
            ...
            if (logoUrl == "Chưa cấu hình") logoUrl = string.Empty;
            ...
            if (string.IsNullOrWhiteSpace(name) && string.IsNullOrWhiteSpace(origin) && string.IsNullOrWhiteSpace(description))
            {
                continue;
            }
            ...
            {
                var existingBrands = await brandReadRepository.GetByNameAsync(name, cancellationToken).ConfigureAwait(false);
                if (existingBrands.Count != 0 || brandsToInsert.Any(b => string.Equals(b.Name, name, StringComparison.OrdinalIgnoreCase)))
                {
            ...
            {
                brandsToInsert.Add(new BrandEntity
                {
                    Name = name,
        ...
        var result = new ImportBrandsResult
        {
            SuccessCount = brandsToInsert.Count,
