    ...
    IVehicleReadRepository vehicleReadRepository) : IRequestHandler<GetCustomerProfile360Query, Result<CustomerProfile360Response>>
{
        ...
        CancellationToken cancellationToken)
    {
        ...
        var repairOrders = !string.IsNullOrWhiteSpace(lead.PhoneNumber)
            ? await repairOrderReadRepository.GetByCustomerPhoneAsync(lead.PhoneNumber, cancellationToken)
                .ConfigureAwait(false)
            : [];
        var vehicles = await vehicleReadRepository.GetByLeadIdAsync(request.LeadId, cancellationToken)
            .ConfigureAwait(false);
        var plateMap = plateDossiers.Items?.Where(pd => pd.OutputId.HasValue).ToDictionary(pd => pd.OutputId!.Value) ??
            new Dictionary<int, PlateDossierResponse>();
        response.Outputs = outputs.Select(
            o =>
            {
                ...
                return new CustomerOutputSummary
                {
                    ...
                    Items =
                        o.OutputInfos?.Where(oi => oi.DeletedAt == null)
                                    .Select(
                                        oi => new CustomerOutputItemSummary
                                    {
                                        Id = oi.Id,
                                        ...
                                        Price = oi.Price,
                                        CoverImageUrl =
                                            oi.ProductVariant?.ProductCollectionPhotos?.FirstOrDefault()?.ImageUrl
                                    })
                                    .ToList() ??
                                []
                };
            ...
            .ToList();
        response.Vehicles = vehicles.Select(
            v => new CustomerVehicleSummary
            {
                Id = v.Id,
            ...
            if (v.NextMaintenanceDate.HasValue)
            {
                ...
                reminders.Add(
                    new CustomerCareReminder
                    {
                        ...
                        Description =
                            $"ODO: {v.NextMaintenanceOdo ?? 0} km - Dự kiến: {v.NextMaintenanceDate.Value:dd/MM/yyyy}",
                        DueDate = v.NextMaintenanceDate,
                        Priority =
                            daysUntil <= 0 ? "urgent" : daysUntil <= 7 ? "high" : daysUntil <= 30 ? "normal" : "low"
                    });
            }
        }
        ...
        foreach (var o in outputs)
        {
            if (o.LastStatusChangedAt.HasValue &&
                o.LastStatusChangedAt < stalledThreshold &&
                !IsTerminalOutput(o.StatusId))
            {
                reminders.Add(
                    new CustomerCareReminder
                    {
                        ...
                        Description =
                            $"Trạng thái: {OrderStatus.GetDisplayName(o.StatusId ?? string.Empty)} - Đã thay đổi: {o.LastStatusChangedAt.Value:dd/MM/yyyy}",
    ...
    private static bool IsTerminalOutput(string? statusId) => string.Equals(
            statusId,
