...
public class SendStoreChatStaffMessageCommandHandler(
    ...
    IUnitOfWork unitOfWork) : IRequestHandler<SendStoreChatStaffMessageCommand, Result<SendStaffMessageResultDto>>
{
    public async Task<Result<SendStaffMessageResultDto>> Handle(
        SendStoreChatStaffMessageCommand request,
        CancellationToken cancellationToken)
    {
        ...
        var assigned = await storeChatUpdateRepository.TryAssignStaffAsync(
            request.SessionId,
            staffId,
            cancellationToken);
        if (!assigned)
