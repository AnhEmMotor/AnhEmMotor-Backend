...
namespace Application.Features.Client.Bookings
{
    ...
    public class GetBookingHistoryHandler : IRequestHandler<GetBookingHistoryQuery, List<BookingHistoryResponse>>
    {
        ...
        public async Task<List<BookingHistoryResponse>> Handle(GetBookingHistoryQuery request, CancellationToken cancellationToken)
        {
    ...
    public class CancelBookingHandler : IRequestHandler<CancelBookingCommand, bool>
    {
        ...
        {
            return await _updateRepo.UpdateStatusAsync(request.Id, BookingStatus.Cancelled.ToString(), request.Reason, DateTime.UtcNow, cancellationToken);
        }
    }
}
