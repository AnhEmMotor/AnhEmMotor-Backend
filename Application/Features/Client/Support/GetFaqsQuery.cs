    ...
    public class GetFaqsHandler : IRequestHandler<GetFaqsQuery, List<FaqResponse>>
    {
        public async Task<List<FaqResponse>> Handle(GetFaqsQuery request, CancellationToken cancellationToken)
        {
            return await Task.FromResult(
                new List<FaqResponse>
                {
                    new FaqResponse(
                    "Lịch bảo dưỡng xe SH là bao lâu?",
                    "Thông thường là mỗi 4000km hoặc 6 tháng một lần."),
                    new FaqResponse(
                    "Có hỗ trợ cứu hộ tại chỗ không?",
                    "AnhEmMotor hỗ trợ cứu hộ 24/7 trong khu vực Biên Hòa.")
                });
        }
    }
