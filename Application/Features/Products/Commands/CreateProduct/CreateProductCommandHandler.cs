...
Error.BadRequest("San pham phai co it nhat mot bien the.", nameof(request.Variants)));
}
...
errors.Add(
Error.NotFound(
$"Danh muc san pham voi ID {request.CategoryId} khong ton tai.",
...
{
errors.Add(
Error.BadRequest($"Thuong hieu voi ID {request.BrandId} khong ton tai.", nameof(request.BrandId)));
}
}
...
errors.Add(
Error.BadRequest(
$"Duong dan (slug) '{slug}' da duoc su dung boi san pham khac.",
"Variants.UrlSlug"));
}
}
}
...
potentialOptionNames.Add("Phien ban");
...
{
errors.Add(Error.BadRequest($"Thuoc tinh '{opt}' khong hop le.", "Variants.OptionValues"));
}
return Result<ProductDetailForManagerResponse?>.Failure(errors);
}
...
{
if (!string.IsNullOrWhiteSpace(variantReq.VariantName) &&
(optionNameMap.TryGetValue("Phien ban", out var versionOptId) ||
...
if (option == null)
return Result<ProductDetailForManagerResponse?>.Failure(
Error.NotFound($"Thuoc tinh voi ID {optionId} khong ton tai.", "Variants.OptionValues"));
...
FuelCapacity = request.FuelCapacity?.ToString(),
...
variantReq.VariantName ?? variantReq.UrlSlug ?? "bien the");
...
if (color is null)
continue;
var colorLabel = $"{variantReq.VariantName ?? variantReq.UrlSlug ?? "bien the"} / {color.ColorName ?? "mau"}";
...
{
if (optionNameMap.TryGetValue("Phien ban", out var versionOptionId) ||
...
return Result<ProductDetailForManagerResponse?>.Failure(
Error.BadRequest(
$"Cac cong nghe sau khong ton tai: {string.Join(", ", invalidIds)}.",
...
$"Moi nha cung cap chi duoc xuat hien mot lan trong {scopeLabel}.",
