...
public class GetVehiclePortfolioQueryHandler(
	...
	IMaintenanceHistoryReadRepository maintenanceRepo)
	: IRequestHandler<GetVehiclePortfolioQuery, Result<VehiclePortfolioResponse?>>
{
	public async Task<Result<VehiclePortfolioResponse?>> Handle(
		GetVehiclePortfolioQuery req,
		CancellationToken ct)
	{
		var q = req.Query.Trim();
		if (string.IsNullOrEmpty(q))
			return Result<VehiclePortfolioResponse?>.Failure(
				[Error.Validation("Query cannot be empty.")]);

		var vehicle = await vehicleRepo.GetVehicleForPortfolioAsync(q, req.QueryType, ct).ConfigureAwait(false);

		if (vehicle is null)
			return Result<VehiclePortfolioResponse?>.Failure(
				[Error.NotFound("No vehicle found matching the query.")]);

		var allHistory = await maintenanceRepo.GetByVehicleIdAsync(
			vehicle.Id, ct, DataFetchMode.All)
		...
		{
			Id = vehicle.Id,
			FullName = vehicle.Lead?.FullName ?? "",
			PhoneNumber = vehicle.Lead?.PhoneNumber ?? "",
			...
			BrandName = vehicle.Product != null && vehicle.Product.Brand != null
		...
		var historyResponse = historyItems.Select(h =>
		{
			List<PortfolioPartItem> details = new();
			try
			{
				var parsed = JsonSerializer.Deserialize<PartsJsonWrapper>(h.PartsJson ?? "");
				if (parsed?.Parts != null)
				{
					foreach (var p in parsed.Parts)
					{
						details.Add(new PortfolioPartItem
						{
							Type = "Part",
