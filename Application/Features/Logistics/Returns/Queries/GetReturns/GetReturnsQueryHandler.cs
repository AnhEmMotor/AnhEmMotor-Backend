...
namespace Application.Features.Logistics.Returns.Queries.GetReturns
{
    ...
    : IRequestHandler<GetReturnsQuery, List<ReturnOrderResponse>>
    {
        public async Task<List<ReturnOrderResponse>> Handle(GetReturnsQuery request, CancellationToken cancellationToken)
        {
            var returns = await parcelDeliveryOrderReadRepository.GetReturnedAsync(cancellationToken).ConfigureAwait(false);
            ...
            .Select(x => new ReturnOrderResponse
            {
                Id = x.Id,
        ...
        {
            if (order.ReturnAction != null) return ReturnOrderStatus.Completed;
            if (order.InspectedAt != null) return ReturnOrderStatus.Inspecting;
