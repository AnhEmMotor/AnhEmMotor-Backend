...
public class GetCarriersQueryHandler(ICarrierPartnerReadRepository carrierPartnerReadRepository) : IRequestHandler<GetCarriersQuery, Result<CarrierPartnerResponse>>
{
    public async Task<Result<CarrierPartnerResponse>> Handle(
        ...
        CancellationToken cancellationToken)
    {
        var items = (await carrierPartnerReadRepository.GetAllAsync(cancellationToken).ConfigureAwait(false))
            .Select(
                x => new CarrierPartnerItemResponse
                {
                    Id = x.Id,
