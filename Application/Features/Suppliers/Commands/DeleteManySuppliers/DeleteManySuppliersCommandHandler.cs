        ...
        IUnitOfWork unitOfWork) : IRequestHandler<DeleteManySuppliersCommand, Result>
    {
        public async Task<Result> Handle(DeleteManySuppliersCommand request, CancellationToken cancellationToken)
        {
            ...
            foreach (var id in request.Ids)
            {
                ...
                var supplierDebts = await supplierDebtRepository.GetBySupplierIdAsync(id, cancellationToken).ConfigureAwait(false);
                if (supplierDebts.Any(sd => sd.InventoryReceipt != null && InventoryReceiptStatus.IsCanEdit(sd.InventoryReceipt.StatusId)))
                {
                    return Result.Failure(Error.Conflict($"Cannot delete supplier with Id {id} due to working InventoryReceipt receipts."));
                }
