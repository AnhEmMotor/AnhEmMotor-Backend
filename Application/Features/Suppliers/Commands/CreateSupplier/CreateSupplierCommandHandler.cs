...
public sealed class CreateSupplierCommandHandler(ISupplierReadRepository supplierReadRepository, ISupplierInsertRepository supplierInsertRepository, IUnitOfWork unitOfWork) : IRequestHandler<CreateSupplierCommand, Result<SupplierResponse>>
{
    ...
    CancellationToken cancellationToken)
    {
        ...
        {
            var isPhoneDuplicate = await supplierReadRepository.IsPhoneExistsAsync(request.Phone, null, cancellationToken)
                                                       .ConfigureAwait(false);
            if (isPhoneDuplicate)
            {
        ...
        {
            var isTaxIdDuplicate = await supplierReadRepository.IsTaxIdExistsAsync(request.TaxIdentificationNumber, null, cancellationToken)
                                                       .ConfigureAwait(false);
            if (isTaxIdDuplicate)
            {
