...
public sealed class UpdateManySupplierStatusCommandValidator : AbstractValidator<UpdateManySupplierStatusCommand>
{
    public UpdateManySupplierStatusCommandValidator()
    {
        ...
        RuleFor(x => x.StatusId)
            ...
            .Must(statusId => SupplierStatusConstants.IsValid(statusId))
            .WithMessage(
                (command, statusId) => $"Trạng thái '{statusId}' không hợp lệ. Chỉ chấp nhận: {string.Join(", ", SupplierStatusConstants.AllowedValues)}");
