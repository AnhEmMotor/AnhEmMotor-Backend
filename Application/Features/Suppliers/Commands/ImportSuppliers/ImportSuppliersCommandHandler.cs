    ...
    IConfiguration configuration) : IRequestHandler<ImportSuppliersCommand, Result<ImportSuppliersResult>>
{
    public async Task<Result<ImportSuppliersResult>> Handle(ImportSuppliersCommand request, CancellationToken cancellationToken)
    {
        ...
        for (int i = 5; i <= rowCount; i++)
        {
            ...
            var matchKey = Domain.Constants.PartnerType.GetKeyFromName(partnerTypeIdRaw);
            ...
            {
                if (!rowErrors.Any(e => e.Contains("NotEmpty", StringComparison.OrdinalIgnoreCase) || e.Contains("empty", StringComparison.OrdinalIgnoreCase)))
                {
            ...
            {
                var nameExists = await supplierReadRepository.IsNameExistsAsync(name, null, cancellationToken).ConfigureAwait(false);
                if (nameExists || suppliersToInsert.Any(b => string.Equals(b.Name, name, StringComparison.OrdinalIgnoreCase)))
                {
            ...
            {
                failedRowsData.Add((partnerTypeIdRaw, name, phone, email, taxId, address, notes, string.Join(", ", rowErrors)));
            }
            else
            {
                suppliersToInsert.Add(new SupplierEntity
                {
                    Name = name,
        ...
        if (failedRowsData.Count > 0)
        {
            ...
            // Generate File 1 (Without Reason)
            using (var wb1 = new XLWorkbook())
            {
                ...
                typeValidation1.List(Domain.Constants.PartnerType.ExcelValidationList);
            ...
            using (var wb2 = new XLWorkbook())
            {
                ...
                string[] headers2 = { "Loại đối tác", "Tên đối tác", "Điện thoại", "Email", "Mã số thuế", "Địa chỉ", "Ghi chú", "Lý Do Lỗi" };
                ...
                typeValidation2.List(Domain.Constants.PartnerType.ExcelValidationList);
