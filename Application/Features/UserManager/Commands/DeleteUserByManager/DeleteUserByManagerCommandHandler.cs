...
{
    public async Task<Result> Handle(DeleteUserByManagerCommand request, CancellationToken cancellationToken)
    {
        ...
        if (!succeeded)
        {
            var errorList = errors?.Any() == true ? errors.Select(e => $"DeleteError: {e}") : ["Failed to delete user."];
