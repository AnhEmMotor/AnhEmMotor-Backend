    ...
    IUnitOfWork unitOfWork) : IRequestHandler<UpdateInputCommand, Result<InputDetailResponse?>>
{
        ...
        CancellationToken cancellationToken)
    {
        ...
        if (Domain.Constants.Input.InputStatus.IsCannotEdit(input.StatusId))
        {
            if (request.Products.Count != 0)
            {
                return Error.BadRequest(
                    "Không du?c ch?nh s?a s?n ph?m trong phi?u nh?p dã hoàn thành ho?c dã h?y.",
                    "Products");
            }
            if (request.SupplierId != null)
            {
                return Error.BadRequest(
                    "Không du?c ch?nh s?a mã nhà cung c?p trong phi?u nh?p dã hoàn thành ho?c dã h?y.",
                    "Products");
            }
        }
        if (request.SupplierId.HasValue && request.SupplierId != input.SupplierId)
        {
            ...
            {
                return Error.BadRequest("Nhà cung c?p không h?p l? ho?c không còn ho?t d?ng.", "SupplierId");
            }
        }
