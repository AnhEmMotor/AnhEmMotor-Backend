...
public class GetWarrantyClaimsListQueryHandler(
 IWarrantyClaimReadRepository readRepo) : IRequestHandler<GetWarrantyClaimsListQuery, Result<PagedResult<WarrantyClaimResponse>>>
{
 public async Task<Result<PagedResult<WarrantyClaimResponse>>> Handle(GetWarrantyClaimsListQuery req, CancellationToken ct)
 {
  ...
  IEnumerable<Domain.Entities.WarrantyClaim> paged;
 ...
 private static IEnumerable<Domain.Entities.WarrantyClaim> ApplySorting(IEnumerable<Domain.Entities.WarrantyClaim> query, string sorts)
 {
  var parts = sorts.Split(',', StringSplitOptions.RemoveEmptyEntries | StringSplitOptions.TrimEntries);
  IOrderedEnumerable<Domain.Entities.WarrantyClaim>? ordered = null;
  bool first = true;
 ...
 private static WarrantyClaimResponse Map(Domain.Entities.WarrantyClaim c)
 {
