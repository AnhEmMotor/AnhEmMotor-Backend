    ...
    INewsReadRepository newsReadRepository) : IRequestHandler<GetNewsCommentsQuery, Result<List<NewsCommentResponse>>>
{
        ...
        CancellationToken cancellationToken)
    {
        ...
        var newsSlugs = comments.Where(
            c => !c.NewsId.HasValue && c.ArticleType == "news" && !string.IsNullOrEmpty(c.ArticleSlug))
            .Select(c => c.ArticleSlug!)
            .Distinct()
            .ToList();
        ...
        var newsDictBySlug = newsList.Where(n => !string.IsNullOrEmpty(n.Slug))
            .GroupBy(n => n.Slug)
            .ToDictionary(g => g.Key, g => g.First());
        string? GetTitle(NewsComment c)
        {
            if (c.NewsId.HasValue && newsDictById.TryGetValue(c.NewsId.Value, out var n))
                return n.Title;
            if (!c.NewsId.HasValue &&
                c.ArticleType == "news" &&
                !string.IsNullOrEmpty(c.ArticleSlug) &&
                newsDictBySlug.TryGetValue(c.ArticleSlug, out var ns))
                return ns.Title;
            return null;
        }
        ...
        {
            if (c.NewsId.HasValue && newsDictById.TryGetValue(c.NewsId.Value, out var n))
                return n.CoverImageUrl;
            if (!c.NewsId.HasValue &&
                c.ArticleType == "news" &&
                !string.IsNullOrEmpty(c.ArticleSlug) &&
                newsDictBySlug.TryGetValue(c.ArticleSlug, out var ns))
                return ns.CoverImageUrl;
            return null;
        }
