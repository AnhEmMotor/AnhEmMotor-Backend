...
public sealed class GetNewsBySlugQueryHandler(INewsReadRepository repository) 
    : IRequestHandler<GetNewsBySlugQuery, Result<NewsResponse>>
{
    public async Task<Result<NewsResponse>> Handle(
        GetNewsBySlugQuery request,
        CancellationToken cancellationToken)
    {
        var news = await repository.GetBySlugAsync(request.Slug, cancellationToken)
