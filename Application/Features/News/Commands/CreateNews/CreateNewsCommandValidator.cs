...
public sealed class CreateNewsCommandValidator : AbstractValidator<CreateNewsCommand>
{
    public CreateNewsCommandValidator()
    {
        ...
        RuleFor(x => x.Slug)
            .NotEmpty().WithMessage("Đường dẫn tĩnh không được để trống.")
            .MaximumLength(255).WithMessage("Slug không được vượt quá 255 ký tự.");
        RuleFor(x => x.CategoryId)
            .NotEmpty().WithMessage("Danh mục bài viết không được để trống.")
            .GreaterThan(0).WithMessage("Danh mục bài viết không hợp lệ.");
