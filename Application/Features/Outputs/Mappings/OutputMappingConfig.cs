...
public sealed class OutputMappingConfig : IRegister
{
    ...
    {
        if (src.ProductVariant?.Product is null) return null;
        ...
        var optionValues = src.ProductVariant.VariantOptionValues?
            ...
            .ToList();
        if (optionValues is null || optionValues.Count == 0) return productName;
    ...
    private static string? MapCoverImageUrl(ProductVariant? variant)
    {
        ...
        if (!string.IsNullOrEmpty(variant.CoverImageUrl)) return variant.CoverImageUrl;
