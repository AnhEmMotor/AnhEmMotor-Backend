...
public sealed class GetWardsQueryHandler(IShippingService shippingService)
    : IRequestHandler<GetWardsQuery, Result<object>>
{
        ...
        try
        {
            ...
            if (root.TryGetProperty("data", out var dataElement) && dataElement.ValueKind == JsonValueKind.Array)
            {
                var wards = dataElement.EnumerateArray()
                    .Select(w => new
                    {
                        WardCode = w.TryGetProperty("_id", out var codeProp) ? codeProp.GetInt32().ToString() : string.Empty,
