...
public class GetAllRolesQueryHandler(
    ...
    IAsyncQueryableExecuter asyncExecuter) : IRequestHandler<GetAllRolesQuery, List<RoleSelectResponse>>
{
    public async Task<List<RoleSelectResponse>> Handle(GetAllRolesQuery request, CancellationToken cancellationToken)
    {
        var roles = await asyncExecuter.ToListAsync(roleManager.Roles
