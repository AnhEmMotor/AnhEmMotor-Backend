...
public class AddLeadActivityCommandHandler(ILeadWriteRepository leadWriteRepository, ILeadReadRepository leadReadRepository) : IRequestHandler<AddLeadActivityCommand, int>
{
    public async Task<int> Handle(AddLeadActivityCommand request, CancellationToken cancellationToken)
    {
        ...
        {
            if (desc.Contains("không nghe máy") || desc.Contains("missed")) scoreDelta = -10;
            else scoreDelta = 10;
        }
