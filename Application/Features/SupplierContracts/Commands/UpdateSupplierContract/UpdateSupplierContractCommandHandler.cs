...
) : IRequestHandler<UpdateSupplierContractCommand, Result<SupplierContractResponse>>
{
    public async Task<Result<SupplierContractResponse>> Handle(
        UpdateSupplierContractCommand request,
        CancellationToken cancellationToken)
    {
        ...
        bool statusBeingChanged = !string.IsNullOrWhiteSpace(request.Request.Status) &&
            string.Compare(request.Request.Status, entity.Status) != 0;
        if (isCurrentlyActive && !statusBeingChanged)
        {
            ...
            {
                return Result<SupplierContractResponse>.Failure(
                    "Cannot modify financial terms of an active contract. Please create an addendum.");
            }
        }
        var auditLogs = new List<SupplierContractAuditLog>();
        if (!string.IsNullOrWhiteSpace(request.Request.ContractNumber) &&
            string.Compare(request.Request.ContractNumber, entity.ContractNumber) != 0)
        {
            var isDuplicate = await readRepo.IsContractNumberExistsAsync(
            ...
            auditLogs.Add(
                new SupplierContractAuditLog
                {
                    Action = "Update",
                    Details =
                        $"Contract number changed from {entity.ContractNumber} to {request.Request.ContractNumber}",
                    ChangedBy = "system",
        ...
        if (request.Request.SupplierId.HasValue)
            entity.SupplierId = request.Request.SupplierId;
        if (request.Request.ContractFilePath != null)
            entity.ContractFilePath = request.Request.ContractFilePath;
        if (request.Request.EffectiveDate != default)
            entity.EffectiveDate = request.Request.EffectiveDate;
        if (request.Request.ExpirationDate.HasValue)
            entity.ExpirationDate = request.Request.ExpirationDate;
        if (request.Request.ContractValue > 0)
            entity.ContractValue = request.Request.ContractValue;
        ...
        if (request.Request.Terms != null)
            entity.Terms = request.Request.Terms;
        ...
        if (request.Request.PaymentWindowDays.HasValue)
            entity.PaymentWindowDays = request.Request.PaymentWindowDays;
        if (request.Request.BankAccountNumber != null)
            entity.BankAccountNumber = request.Request.BankAccountNumber;
        if (request.Request.BankName != null)
            entity.BankName = request.Request.BankName;
        if (request.Request.MinimumVolumePerMonth.HasValue)
            entity.MinimumVolumePerMonth = request.Request.MinimumVolumePerMonth;
        ...
        if (request.Request.ParentContractId.HasValue)
