    ...
    : IRequestHandler<GetSupplierContractStatisticsQuery, Result<SupplierContractStatisticsResponse>>
{
        ...
        CancellationToken cancellationToken)
    {
        ...
        var active = await repository.CountByStatusAsync(SupplierContractStatus.Active, cancellationToken).ConfigureAwait(false);
        var pending = await repository.CountByStatusAsync(SupplierContractStatus.PendingApproval, cancellationToken).ConfigureAwait(false);
        var expired = await repository.CountByStatusAsync(SupplierContractStatus.Expired, cancellationToken).ConfigureAwait(false);
