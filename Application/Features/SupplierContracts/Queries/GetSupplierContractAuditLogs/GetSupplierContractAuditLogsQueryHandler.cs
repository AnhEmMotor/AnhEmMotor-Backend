...
public class GetSupplierContractAuditLogsQueryHandler(ISupplierContractReadRepository repository) : IRequestHandler<GetSupplierContractAuditLogsQuery, Result<List<SupplierContractAuditLogResponse>>>
{
    public async Task<Result<List<SupplierContractAuditLogResponse>>> Handle(
        GetSupplierContractAuditLogsQuery request,
        CancellationToken cancellationToken)
    {
        var contract = await repository.GetByIdAsync(request.SupplierContractId, cancellationToken, DataFetchMode.All)
