...
{
    public async Task<Result<ImportInventoryReceiptsResult>> Handle(ImportInventoryReceiptsCommand request, CancellationToken cancellationToken)
    {
        ...
        if (worksheet == null)
        {
            return Result<ImportInventoryReceiptsResult>.Failure(Error.BadRequest("Excel file does not contain any worksheet."));
        }
        ...
        for (int i = 5; i <= rowCount; i++)
        {
            ...
            parsedRows.Add((prIdStr, productName, variantName, colorName, remQtyStr, qtyStr, vin, engine, generalNote, i));
        ...
        {
            prItems = await purchaseRequestReadRepository.GetItemsByPurchaseRequestIdsAsync(uniquePrIds, cancellationToken).ConfigureAwait(false);
        }
        ...
        foreach (var r in parsedRows)
        {
            ...
            if (string.IsNullOrWhiteSpace(r.PrIdStr)) rowErrors.Add("Thiếu ID Yêu cầu mua hàng.");
            if (string.IsNullOrWhiteSpace(r.QtyStr)) rowErrors.Add("Thiếu số lượng.");
            if (string.IsNullOrWhiteSpace(r.ProductName)) rowErrors.Add("Thiếu tên sản phẩm.");

            int prId = 0, qty = 0;

            if (!string.IsNullOrWhiteSpace(r.PrIdStr) && !int.TryParse(r.PrIdStr, out prId)) rowErrors.Add("ID YCMH không hợp lệ.");
            if (!string.IsNullOrWhiteSpace(r.QtyStr) && !int.TryParse(r.QtyStr, out qty)) rowErrors.Add("Số lượng không hợp lệ.");

            int prItemId = 0;
            if (prId > 0 && rowErrors.Count == 0)
            {
                var matchedItem = prItems.FirstOrDefault(x => 
                    x.PurchaseRequestId == prId && 
                    x.ProductVariant?.Product?.Name == r.ProductName && 
                    x.ProductVariant?.VariantName == r.VariantName && 
                    (string.IsNullOrWhiteSpace(r.ColorName) ? x.ProductVariantColor == null : x.ProductVariantColor?.ColorName == r.ColorName));
            ...
            {
                failedRowsData.Add((r.PrIdStr, r.ProductName, r.VariantName, r.ColorName, r.RemQtyStr, r.QtyStr, r.Vin, r.Engine, r.Note, string.Join(", ", rowErrors)));
            }
            else
            {
                ...
                {
                    existingInfo.Vehicles.Add(new Vehicle
                    {
                        VinNumber = r.Vin,
        ...
        if (failedRowsData.Count > 0)
        {
            ...
            string[] headers1 = { "Mã yêu cầu mua hàng", "Tên sản phẩm", "Tên biến thể sản phẩm", "Tên biến thể màu sắc của sản phẩm", "Số lượng còn lại", "Số lượng nhập", "Số khung", "Số máy" };
