...
{
    public async Task<Result<byte[]>> Handle(GetImportInventoryReceiptTemplateQuery request, CancellationToken cancellationToken)
    {
        ...
        string[] headers = { "Mã yêu cầu mua hàng", "Tên sản phẩm", "Tên biến thể sản phẩm", "Tên biến thể màu sắc của sản phẩm", "Số lượng còn lại", "Số lượng nhập", "Số khung", "Số máy" };
        ...
        var items = await purchaseRequestReadRepository.GetItemsByPurchaseRequestIdsAsync(
            new[] { request.PurchaseRequestId }, cancellationToken);
            ...
            foreach (var item in items)
            {
                ...
                if (managementType == "vin_number")
                {
                    for (int j = 0; j < remainingCount; j++)
                    {
                        ...
                        worksheet.Cell(currentRow, 7).Style.Fill.SetBackgroundColor(XLColor.Yellow); // Số khung
                        worksheet.Cell(currentRow, 8).Style.Fill.SetBackgroundColor(XLColor.Yellow); // Số máy
                        currentRow++;
                    }
                }
                else
                {
                    ...
                    worksheet.Cell(currentRow, 6).Style.Fill.SetBackgroundColor(XLColor.Yellow); // Số lượng nhập
                    currentRow++;
                }
        }
    ...
    private void FillRowData(IXLWorksheet worksheet, int row, Domain.Entities.PurchaseRequestItem item, int remainingCount, bool showRemainingCount)
    {
        worksheet.Cell(row, 1).Value = item.PurchaseRequestId;
        worksheet.Cell(row, 2).Value = item.ProductVariant?.Product?.Name ?? "";
        worksheet.Cell(row, 3).Value = item.ProductVariant?.VariantName ?? "";
        worksheet.Cell(row, 4).Value = item.ProductVariantColor?.ColorName ?? "";
