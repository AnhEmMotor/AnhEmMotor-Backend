...
public class GetInventoryReceiptAuditLogsQueryHandler(IInventoryReceiptReadRepository repository) : IRequestHandler<GetInventoryReceiptAuditLogsQuery, Result<List<InventoryReceiptAuditLogResponse>>>
{
        ...
        CancellationToken cancellationToken)
    {
        ...
        var result = logs.Select(
            l => new InventoryReceiptAuditLogResponse
            {
                Id = l.Id,
                ...
                .Where(il => Math.Abs((il.CreatedAt.GetValueOrDefault() - l.ChangedAt).TotalSeconds) <= 5)
                            .Select(
                                il =>
                                {
                                    ...
                                    var productVariantName = prItem?.ProductVariant != null
                                        ? $"{prItem.ProductVariant.Product?.Name}"
                                        : null;
                                    if (prItem?.ProductVariant != null &&
                                        !string.IsNullOrEmpty(prItem.ProductVariant.VariantName))
                                    {
