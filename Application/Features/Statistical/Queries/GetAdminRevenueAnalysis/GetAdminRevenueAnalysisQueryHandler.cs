...
public class GetAdminRevenueAnalysisQueryHandler(IStatisticalReadRepository repository) : IRequestHandler<GetAdminRevenueAnalysisQuery, Result<AdminRevenueAnalysisResponse>>
{
        ...
        CancellationToken cancellationToken)
    {
        ...
        var tableData = await repository.GetDailyRevenueTableDataAsync(start, end, cancellationToken)
            .ConfigureAwait(false);
        var topProducts = await repository.GetTopProductsByRevenueAsync(start, end, 5, cancellationToken)
            .ConfigureAwait(false);
