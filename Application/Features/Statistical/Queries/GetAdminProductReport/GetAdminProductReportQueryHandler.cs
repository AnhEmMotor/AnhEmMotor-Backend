...
public sealed class GetAdminProductReportQueryHandler(IStatisticalReadRepository repository) : IRequestHandler<GetAdminProductReportQuery, Result<AdminProductReportResponse>>
{
        ...
        CancellationToken cancellationToken)
    {
        ...
        decimal deadStockValue = deadStock != null ? (decimal)(deadStock.StockQuantity * deadStock.SellPrice) : 0; // Ước tính bằng SellPrice vì DTO không có CostPrice, hoặc ta có thể coi đây là giá trị vốn nếu có. Ở Frontend hiển thị "Giá trị".

        double avgTurnover = 0;
        if (tableData.Any(p => p.StockQuantity > 0))
        {
            avgTurnover = tableData.Where(p => p.StockQuantity > 0).Average(p => (double)p.SoldCount30Days / p.StockQuantity);
        }
        ...
        var topRevenue = tableData
            ...
            .Take(5)
            .Select(p => new TopProductRevenueResponse
            {
        ...
        var topProfit = tableData
            ...
            .Take(5)
            .Select(p => new TopProductProfitResponse
            {
                ProductName = p.ProductName,
