    ...
    IUnitOfWork uow) : IRequestHandler<UpdateBookingAppointmentCommand, Result<bool>>
{
    public async Task<Result<bool>> Handle(UpdateBookingAppointmentCommand req, CancellationToken ct)
    {
        ...
        if (req.ServiceType is not null)
            entity.ServiceType = req.ServiceType;
        if (req.PreferredDate.HasValue)
            entity.PreferredDate = req.PreferredDate.Value;
        if (req.PreferredTimeSlot is not null)
            entity.PreferredTimeSlot = req.PreferredTimeSlot;
        if (req.AppointmentAt.HasValue)
            entity.AppointmentAt = req.AppointmentAt.Value;
        if (req.Showroom is not null)
            entity.Showroom = req.Showroom;
