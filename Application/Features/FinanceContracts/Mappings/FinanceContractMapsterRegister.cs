...
public class FinanceContractMapsterRegister : IRegister
{
	public void Register(TypeAdapterConfig config)
	{
		config.NewConfig<FinanceContract, FinanceContractDetailResponse>()
			...
			.Map(dest => dest.FinancialPartner, src => new PartnerResponse
			{
				Name = src.BankName
			})
			.Map(dest => dest.CreditPackage, src => new CreditPackageResponse
			{
				PrincipalAmount = src.LoanAmount,
				TermMonths = src.TermMonths,
				InterestRateRange = $"{src.InterestRate}%",
			})
			.Map(dest => dest.Disbursement, src => new DisbursementResponse
			{
			...
			.Map(dest => dest.Cavet, src => new CavetResponse
			{
				State = MapCavetState(src.CavetLocation),
			});
	}

	private static string MapCavetState(string? location) =>
