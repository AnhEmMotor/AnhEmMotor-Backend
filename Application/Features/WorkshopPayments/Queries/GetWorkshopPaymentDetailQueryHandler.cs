...
public class GetWorkshopPaymentDetailQueryHandler(
    IWorkshopPaymentReadRepository repo) : IRequestHandler<GetWorkshopPaymentDetailQuery, Result<WorkshopPaymentResponse?>>
{
    public async Task<Result<WorkshopPaymentResponse?>> Handle(GetWorkshopPaymentDetailQuery req, CancellationToken ct)
    {
        ...
        if (entity is null)
            return Result<WorkshopPaymentResponse?>.Failure([Error.NotFound($"Không tìm thấy phiếu thu id={req.Id}", "Id")]);

        return Result<WorkshopPaymentResponse?>.Success(new WorkshopPaymentResponse
        {
            Id = entity.Id,
