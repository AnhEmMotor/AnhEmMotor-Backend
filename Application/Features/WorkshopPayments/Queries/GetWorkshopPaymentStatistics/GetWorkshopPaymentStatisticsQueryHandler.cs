...
public class GetWorkshopPaymentStatisticsQueryHandler(
    IWorkshopPaymentReadRepository repo) : IRequestHandler<GetWorkshopPaymentStatisticsQuery, Result<WorkshopPaymentStatisticsResponse>>
{
        ...
        CancellationToken cancellationToken)
    {
        ...
        var paidToday = payments.Where(p => p.PaymentStatus == "Paid" && p.PaidAt.HasValue && p.PaidAt.Value >= today).ToList();
        var monthPaid = payments.Where(p => p.PaymentStatus == "Paid" && p.PaidAt.HasValue && p.PaidAt.Value >= monthStart).ToList();
