...
/// <summary>
/// Phân loại ý định duyệt/huỷ kế hoạch từ chat — khớp CHÍNH XÁC toàn bộ câu (không phải substring như
/// SteeringClassifier), vì đây là hành động có hậu quả: không khớp thì rơi xuống nhánh LLM diễn giải (an toàn hơn tự
/// suy đoán "gần đúng").
/// </summary>
public static class PlanChatClassifier
{
    ...
    public static string? Classify(string text)
    {
        ...
        if (ApproveExact.Contains(normalized))
            return "approve";
        if (RejectExact.Contains(normalized))
            return "reject";
        return null;
    }
}
