...
/// <summary>
/// Thay cho nút Duyệt/Huỷ trên PlanCard (Stage 10.9): mọi tin nhắn gõ trong lúc plan Drafting/Ready đi qua đây thay vì
/// SendSteering (vốn cố ý từ chối AwaitingApproval vì graph đã kết thúc — route plan→END, không interrupt). Chỉ điều
/// phối, KHÔNG viết lại nghiệp vụ đã có ở Approve/Reject/UpdateChatPlan — mọi ownership/version-conflict/permission vẫn
/// do 3 handler đó tự kiểm tra khi được gọi lại qua ISender.
/// </summary>
...
{
    public async Task<Result<PlanChatResultDto>> Handle(
        SendPlanChatMessageCommand request,
        CancellationToken cancellationToken)
    {
        ...
        if (plan == null || plan.Run?.Session?.UserId != userId)
        {
            return Result<PlanChatResultDto>.Failure(
                Error.NotFound("Plan không tồn tại hoặc không thuộc quyền sở hữu."));
        }
        ...
        chatInsertRepository.AddMessage(
            new ChatMessage
            {
                Id = Guid.NewGuid(),
        ...
        if (request.TargetStepId == null)
        {
            ...
            if (action == "approve")
            {
                var approveResult = await sender.Send(
                    new ApproveChatPlanCommand(request.RunId, plan.Version),
                    cancellationToken);
        ...
        var updateResult = await sender.Send(
            new UpdateChatPlanCommand(request.RunId, plan.Version, operations),
            cancellationToken);
