...
public sealed class UploadMultipleFilesCommandHandler(IFileRepository fileRepository, IMediaFileInsertRepository mediaFileInsertRepository, IUnitOfWork unitOfWork)
    : IRequestHandler<UploadMultipleFilesCommand, (List<FileResponse>? Data, ErrorResponse? Error)>
{
    ...
    public async Task<(List<FileResponse>? Data, ErrorResponse? Error)> Handle(UploadMultipleFilesCommand request, CancellationToken cancellationToken)
    {
        var tasks = request.Files.Select(file => ProcessAndSaveFileToDiskAsync(file, request.BaseUrl, cancellationToken));
