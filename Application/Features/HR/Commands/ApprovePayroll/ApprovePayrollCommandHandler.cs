...
public class ApprovePayrollCommandHandler(ICommissionReadRepository commissionRepository, IUnitOfWork unitOfWork) : IRequestHandler<ApprovePayrollCommand, Result>
{
    public async Task<Result> Handle(ApprovePayrollCommand request, CancellationToken cancellationToken)
    {
        ...
        var allRecords = await commissionRepository
            .GetRecordsByStatusAsync(CommissionStatus.Confirmed, request.EmployeeId, cancellationToken)
            .ConfigureAwait(false);
        var periodStart = new DateTime(request.Year, request.Month, 1);
        var periodEnd = periodStart.AddMonths(1);
        var periodRecords = allRecords
            .Where(r => r.DateEarned >= periodStart && r.DateEarned < periodEnd)
            .ToList();
        if (periodRecords.Count == 0)
        {
            var monthLabel = $"{request.Month:00}/{request.Year}";
            return Result.Failure($"Không có khoản hoa hồng nào cần duyệt chi cho tháng {monthLabel}.");
        }
