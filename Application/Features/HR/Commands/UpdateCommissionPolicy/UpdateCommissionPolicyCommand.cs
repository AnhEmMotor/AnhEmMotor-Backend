...
public sealed class UpdateCommissionPolicyCommandHandler(IApplicationDBContext context)
    : IRequestHandler<UpdateCommissionPolicyCommand, Result>
{
    public async Task<Result> Handle(UpdateCommissionPolicyCommand request, CancellationToken cancellationToken)
    {
        ...
        if (policy == null) return Result.Failure("Không tìm thấy chính sách.");
        ...
        var auditLog = new CommissionPolicyAuditLog
        {
            ...
            Description = $"Cập nhật định mức: {policy.Name}. Giá trị mới: {policy.Value}{(policy.Type == "Percentage" ? "%" : "đ")}",
