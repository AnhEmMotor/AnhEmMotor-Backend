...
{
    public class GetPurchaseRequestAuditLogsQueryHandler(IPurchaseRequestReadRepository readRepository, IProductVariantReadRepository variantRepository) : IRequestHandler<GetPurchaseRequestAuditLogsQuery, Result<List<PurchaseRequestAuditLogResponse>>>
    {
        public async Task<Result<List<PurchaseRequestAuditLogResponse>>> Handle(GetPurchaseRequestAuditLogsQuery request, CancellationToken cancellationToken)
        {
            ...
            var variantIds = itemLogs
                ...
                .Distinct()
                .ToList();
            var variants = variantIds.Any() ? await variantRepository.GetByIdAsync(variantIds, cancellationToken, DataFetchMode.All).ConfigureAwait(false) : new List<Domain.Entities.ProductVariant>();
            ...
            string? GetVariantFullName(int? variantId, int? colorId)
            {
                ...
                var variantName = string.IsNullOrEmpty(variant.VariantName) ? "" : $" - {variant.VariantName}";
                var colorName = "";
            ...
            var result = logs.Select(l => new PurchaseRequestAuditLogResponse
            {
                Id = l.Id,
                ...
                ItemLogs = itemLogs
                    ...
                    .Select(il => new PurchaseRequestItemAuditLogResponse
                {
                    Id = il.Id,
                    ...
                    OldProductVariantName = GetVariantFullName(il.OldProductVariantId, il.OldProductVariantColorId),
                    NewProductVariantName = GetVariantFullName(il.NewProductVariantId, il.NewProductVariantColorId),
