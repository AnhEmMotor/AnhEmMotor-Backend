...
{
    public async Task<Result<ImportPurchaseRequestsResult>> Handle(ImportPurchaseRequestsCommand request, CancellationToken cancellationToken)
    {
        ...
        if (worksheet == null)
        {
            return Result<ImportPurchaseRequestsResult>.Failure(Error.BadRequest("Excel file does not contain any worksheet."));
        }
        ...
        // Load all required data into memory for validation
        var allProducts = await productReadRepository.GetAllAsync(cancellationToken).ConfigureAwait(false);
        var allVariants = await variantReadRepository.GetAllAsync(cancellationToken, Domain.Constants.DataFetchMode.ActiveOnly).ConfigureAwait(false);
        var allSuppliers = await supplierReadRepository.GetAllAsync(cancellationToken, Domain.Constants.DataFetchMode.ActiveOnly).ConfigureAwait(false);
        ...
        for (int i = 5; i <= rowCount; i++)
        {
            ...
            if (string.IsNullOrWhiteSpace(tempCode)) rowErrors.Add("Thiếu mã phiếu.");
            if (string.IsNullOrWhiteSpace(productName)) rowErrors.Add("Thiếu tên sản phẩm.");
            if (string.IsNullOrWhiteSpace(variantName)) rowErrors.Add("Thiếu tên biến thể.");
            if (string.IsNullOrWhiteSpace(qtyStr)) rowErrors.Add("Thiếu số lượng.");
            if (string.IsNullOrWhiteSpace(supplierName)) rowErrors.Add("Thiếu tên nhà cung cấp.");

            int qty = 0;
            if (!string.IsNullOrWhiteSpace(qtyStr) && !int.TryParse(qtyStr, out qty)) rowErrors.Add("Số lượng không hợp lệ.");
            ...
            {
                matchedProduct = allProducts.FirstOrDefault(p => string.Equals(p.Name, productName, StringComparison.OrdinalIgnoreCase));
                ...
                {
                    matchedVariant = allVariants.FirstOrDefault(v => v.ProductId == matchedProduct.Id && string.Equals(v.VariantName, variantName, StringComparison.OrdinalIgnoreCase));
                    ...
                    else
                    {
                        ...
                        {
                            matchedColor = matchedVariant.ProductVariantColors?.FirstOrDefault(c => string.Equals(c.ColorName, colorName, StringComparison.OrdinalIgnoreCase));
                ...
                matchedSupplier = allSuppliers.FirstOrDefault(s => string.Equals(s.Name, supplierName, StringComparison.OrdinalIgnoreCase));
                if (matchedSupplier == null)
                {
                    rowErrors.Add("Tên nhà cung cấp không tồn tại.");
                }

                if (matchedVariant != null && matchedSupplier != null && (string.IsNullOrWhiteSpace(colorName) || matchedColor != null))
                {
                    int? colorId = matchedColor?.Id;
                    matchedQuotation = allQuotations.FirstOrDefault(q => q.ProductVariantId == matchedVariant.Id && q.ProductVariantColorId == colorId && q.SupplierId == matchedSupplier.Id);
                }
            }
            ...
            if (rowErrors.Count > 0)
            {
                failedRowsData.Add((tempCode, note, productName, variantName, colorName, qtyStr, supplierName, string.Join(", ", rowErrors)));
            }
            else
            {
                ...
                tempCodeGroup[tempCode].Add(new PurchaseRequestItem
                {
        ...
        if (failedRowsData.Count > 0)
        {
            ...
            var wwwrootPath = !string.IsNullOrWhiteSpace(customUploadPath) 
                ? (Path.IsPathRooted(customUploadPath) ? customUploadPath : Path.Combine(Directory.GetCurrentDirectory(), customUploadPath))
                : Path.Combine(Directory.GetCurrentDirectory(), "wwwroot");

            var errorsDir = Path.Combine(wwwrootPath, "import-errors");
            if (!Directory.Exists(errorsDir)) Directory.CreateDirectory(errorsDir);
            ...
            string[] headers1 = { "Mã phiếu", "Ghi chú", "Tên sản phẩm", "Tên biến thể sản phẩm", "Tên biến thể màu sắc của sản phẩm (nếu có)", "Số lượng yêu cầu", "Tên nhà cung cấp" };
            ...
            using (var wb2 = new XLWorkbook())
            {
                ...
                for (int i = 0; i < headers1.Length; i++) ws2.Cell(1, i + 1).Value = headers1[i];
