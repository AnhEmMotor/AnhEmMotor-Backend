...
{
    public sealed class UploadCvCommandHandler(IFileInsertService fileInsertService)
        : IRequestHandler<UploadCvCommand, Result<string>>
    {
        public async Task<Result<string>> Handle(UploadCvCommand request, CancellationToken cancellationToken)
        {
            ...
            {
                return Result<string>.Failure("Định dạng tệp không được hỗ trợ. Chỉ cho phép tải lên tệp PDF, Word (doc, docx) và hình ảnh.");
            }
