    ...
    where TResponse : class
{
        ...
        CancellationToken cancellationToken)
    {
        ...
        if(failures.Count > 0)
        {
            var errors = failures.Select(f => Error.Validation(f.ErrorMessage, f.PropertyName)).ToList();
            
            var responseType = typeof(TResponse);
            if(responseType.IsGenericType && responseType.GetGenericTypeDefinition() == typeof(Result<>))
            {
                var failureMethod = responseType.GetMethod(nameof(Result<>.Failure), new[] { typeof(List<Error>) });
