...
public class Product
{
    ...
    public async Task GetProducts_NoAuth_ReturnsSuccess()
    {
        ...
        _senderMock.Setup(m => m.Send(It.IsAny<GetProductsListQuery>(), It.IsAny<CancellationToken>()))
            .ReturnsAsync(new Domain.Primitives.PagedResult<Application.ApiContracts.Product.Responses.ProductDetailResponse>(
                [], 0, 1, 10));
    ...
    {
        // Arrange
        _senderMock.Setup(m => m.Send(It.IsAny<GetActiveVariantLiteListForOutputQuery>(), It.IsAny<CancellationToken>()))
            .ThrowsAsync(new UnauthorizedAccessException());
