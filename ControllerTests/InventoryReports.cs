...
namespace ControllerTests
{
    public class InventoryReports
    {
        ...
        public async Task IRP_001_GetSummaryReport_Success()
        {
            var mockResponse = new List<InventoryReportSummaryResponse>
            {
                new()
                {
                    ...
                    ExportedQty = 2,
                    Variants =
                        new List<InventoryReportVariantResponse>
                        {
                            new() { VariantId = 2, ImportedQty = 10, ExportedQty = 2 }
                        }
                }
            };
            var pagedResponse = new PagedResult<InventoryReportSummaryResponse>(mockResponse, 1, 1, 10);
            _mediatorMock.Setup(m => m.Send(It.IsAny<GetInventoryReportSummaryQuery>(), It.IsAny<CancellationToken>()))
                .ReturnsAsync(Result<PagedResult<InventoryReportSummaryResponse>>.Success(pagedResponse));
            var result = await _controller.GetInventoryReportSummaryAsync(
                new GetInventoryReportSummaryQuery(),
        ...
        public async Task IRP_003_GetReportDetail_Success()
        {
            ...
            var result = await _controller.GetInventoryReportDetailAsync(2, 10, CancellationToken.None)
                .ConfigureAwait(true);
            result.Should().BeOfType<OkObjectResult>();
            var okResult = result as OkObjectResult;
