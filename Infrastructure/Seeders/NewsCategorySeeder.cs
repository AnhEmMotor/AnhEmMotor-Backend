...
public static class NewsCategorySeeder
{
    public static async Task SeedAsync(ApplicationDBContext context, CancellationToken cancellationToken)
    {
        ...
        var existingIds = await context.NewsCategories
            .Select(c => c.Id)
            ...
            try
            {
                if (isSqlServer)
                {
                    await context.Database
                        .ExecuteSqlRawAsync("SET IDENTITY_INSERT [dbo].[NewsCategory] ON", cancellationToken)
                        .ConfigureAwait(false);
                }
                ...
                {
                    await context.Database
                        .ExecuteSqlRawAsync("SET IDENTITY_INSERT [dbo].[NewsCategory] OFF", cancellationToken)
                        .ConfigureAwait(false);
                }
