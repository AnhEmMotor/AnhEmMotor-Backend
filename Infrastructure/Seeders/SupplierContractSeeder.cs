...
public static class SupplierContractSeeder
{
    public static async Task SeedAsync(ApplicationDBContext context, CancellationToken cancellationToken)
    {
        ...
        if (supplierHonda != null && hondaVariants.Count > 0)
        {
            ...
            if (!existingContracts.Any(c => c.ContractNumber == contractNumber))
            {
                var contract = new SupplierContract
                {
                    ...
                    Terms = "Cong ty Honda Viet Nam cung cap cac dong xe may chinh hang voi chat luong dam bao. Hop dong co hieu luc trong 2 nam voi cac dieu khoan ve gia ca, chat luong va chinh sach bao hanh theo quy dinh cua nha san xuat.",
                ...
                {
                    contract.ContractItems.Add(new SupplierContractItem
                    {
                        Id = Guid.NewGuid(),
        ...
        if (supplierYamaha != null && yamahaVariants.Count > 0)
        {
            ...
            if (!existingContracts.Any(c => c.ContractNumber == contractNumber))
            {
                var contract = new SupplierContract
                {
                    ...
                    Status = SupplierContractStatus.Active,
                    Terms = "Yamaha Motor Viet Nam cung cap xe may Yamaha chinh hang voi day du chung tu. Hop dong bao gom chinh sach gia si, thoi gian giao hang va che do bao hanh.",
        ...
        if (supplierSuzuki != null && suzukiVariants.Count > 0)
        {
            ...
            if (!existingContracts.Any(c => c.ContractNumber == contractNumber))
            {
                var contract = new SupplierContract
                {
                    ...
                    Terms = "Suzuki Viet Nam cung cap cac dong xe Suzuki voi gia ca canh tranh. Dang cho phe duyet cuoi cung tu phong ke toan.",
        ...
        {
            ("HD-DRAFT-2024-004", supplierHonda?.Id ?? 0, new DateTime(2024, 7, 1), null, 5000000000, SupplierContractStatus.Draft, "Nhap hop dong bo sung phu kien Honda. Chua hoan thien.", "Dang soan thao", 1000000000, 30, "Vietcombank", "1111222233", 20, 1.0m, new DateTimeOffset(2024, 7, 1, 0, 0, 0, TimeSpan.Zero)),
            ("HD-YAMAHA-EXPIRED-2023-001", supplierYamaha?.Id ?? 0, new DateTime(2023, 1, 1), new DateTime(2024, 1, 1, 0, 0, 0), 10000000000, SupplierContractStatus.Expired, "Hop dong cu da het han.", "Hop dong cu da het han", 2000000000, 30, "Techcombank", "7777888899", 60, 2.0m, new DateTimeOffset(2023, 1, 1, 0, 0, 0, TimeSpan.Zero)),
            ("HD-HONDA-COMPLETED-2022-001", supplierHonda?.Id ?? 0, new DateTime(2022, 1, 1), new DateTime(2023, 12, 31, 0, 0, 0), 20000000000, SupplierContractStatus.Completed, "Hop dong da hoan thanh.", "Da hoan thanh toan bo giao dich", 5000000000, 30, "Vietcombank", "1234567890", 150, 4.0m, new DateTimeOffset(2022, 1, 1, 0, 0, 0, TimeSpan.Zero)),
            ("HD-TERMINATED-2023-002", fallbackSupplier?.Id ?? 0, new DateTime(2023, 4, 1), new DateTime(2024, 4, 1, 0, 0, 0), 6000000000, SupplierContractStatus.Terminated, "Hop dong da thanh ly do vi pham dieu khoan.", "Thanh ly som", 1500000000, 45, "ACB", "5555666677", 40, 1.0m, new DateTimeOffset(2023, 4, 1, 0, 0, 0, TimeSpan.Zero)),
        };
