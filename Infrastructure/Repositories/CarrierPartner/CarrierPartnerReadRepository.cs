...
{
    public Task<List<CarrierPartnerEntity>> GetAllAsync(CancellationToken cancellationToken = default) => context.Set<CarrierPartnerEntity>(
        )
        .OrderBy(x => x.Id)
        .ToListAsync(cancellationToken);

    public Task<CarrierPartnerEntity?> GetByIdAsync(int id, CancellationToken cancellationToken = default) => context.Set<CarrierPartnerEntity>(
        )
