using Application.Interfaces.Repositories.WarrantyTerm;
using Infrastructure.DBContexts;

namespace Infrastructure.Repositories.WarrantyTerm;

public class WarrantyTermUpdateRepository(ApplicationDBContext context) : IWarrantyTermUpdateRepository
{
    public void Update(Domain.Entities.WarrantyTerm warrantyTerm)
    {
        context.Set<Domain.Entities.WarrantyTerm>().Update(warrantyTerm);
    }
}
