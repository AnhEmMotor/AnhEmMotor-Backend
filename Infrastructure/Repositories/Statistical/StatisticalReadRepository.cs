...
public class StatisticalReadRepository(ApplicationDBContext context) : IStatisticalReadRepository
{
	...
	public async Task<DashboardKpisResponse> GetDashboardKpisAsync(string period, CancellationToken cancellationToken)
	{
		...
		var lbl = period.ToLower() switch { "today" => "Hom nay", "week" => "Tuan nay", "year" => "Nam nay", _ => "Thang nay" };

		var orders  = await context.OutputOrders.IgnoreQueryFilters().Where(o => o.CreatedAt >= s && o.CreatedAt <= e).CountAsync(cancellationToken);
		var prevOrd = await context.OutputOrders.IgnoreQueryFilters().Where(o => o.CreatedAt >= ps && o.CreatedAt <= pe).CountAsync(cancellationToken);
		var custs   = await context.CustomerContacts.IgnoreQueryFilters().Where(c => c.CreatedAt >= s && c.CreatedAt <= e).CountAsync(cancellationToken);
		var prevCst = await context.CustomerContacts.IgnoreQueryFilters().Where(c => c.CreatedAt >= ps && c.CreatedAt <= pe).CountAsync(cancellationToken);
		var appts   = await context.BookingAppointments.IgnoreQueryFilters().Where(a => a.AppointmentAt >= s && a.AppointmentAt <= e).CountAsync(cancellationToken);
		var pending = await context.OutputOrders.IgnoreQueryFilters()
			.Where(o => o.StatusId == "pending" || o.StatusId == "waiting_deposit" || o.StatusId == "waiting_installment").CountAsync(cancellationToken);
		var overdue = await context.FinanceContracts.IgnoreQueryFilters()
		...
		var lowP    = await context.InventoryOnHands.IgnoreQueryFilters().Where(i => i.StockQty < 10).CountAsync(cancellationToken);
		var comp    = await context.Set<Domain.Entities.CustomerFeedback>().IgnoreQueryFilters().Where(f => f.CreatedAt >= s && f.CreatedAt <= e).CountAsync(cancellationToken);
		var missed  = await context.Set<Domain.Entities.BookingAppointment>().IgnoreQueryFilters()
			.Where(a => a.Status == "no_show" && a.AppointmentAt >= s && a.AppointmentAt <= e).CountAsync(cancellationToken);
