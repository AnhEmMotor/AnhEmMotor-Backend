...
{
    public Task<List<ParcelDeliveryOrderEntity>> GetAllAsync(CancellationToken cancellationToken = default) => context.Set<ParcelDeliveryOrderEntity>(
        )
        .Include(x => x.Items)
        .ToListAsync(cancellationToken);

    public Task<List<ParcelDeliveryOrderEntity>> GetReturnedAsync(CancellationToken cancellationToken = default) => context.Set<ParcelDeliveryOrderEntity>(
        )
        .Where(x => x.Status == ParcelDeliveryStatus.Returned)
        .Include(x => x.Items)
        .ToListAsync(cancellationToken);

    public Task<ParcelDeliveryOrderEntity?> GetByIdAsync(int id, CancellationToken cancellationToken = default) => context.Set<ParcelDeliveryOrderEntity>(
        )
        .Include(x => x.Items)
        ...
        CancellationToken cancellationToken = default)
    {
        ...
        return context.Set<ParcelDeliveryOrderEntity>()
            .Include(x => x.Items)
            .FirstOrDefaultAsync(
                x => string.Compare(x.TrackingNumber, normalized) == 0 ||
                    string.Compare(x.CustomerPhone, normalized) == 0 ||
                    string.Compare(x.OriginalOrderCode, normalized) == 0,
                cancellationToken);
    }
