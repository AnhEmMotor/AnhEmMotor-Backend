...
public partial class StoreChatReadRepository(ApplicationDBContext context) : IStoreChatReadRepository
{
    ...
    private static string? StripHtmlTags(string? content) => string.IsNullOrEmpty(content)
        ? content
        : HtmlTagRegex().Replace(content, string.Empty).Trim();

    public async Task<StoreChatSession?> GetSessionByVisitorKeyAsync(
        string visitorKey,
        CancellationToken cancellationToken = default)
    {
        return await context.StoreChatSessions.FirstOrDefaultAsync(s => s.VisitorKey == visitorKey, cancellationToken);
    }

    public async Task<StoreChatSession?> GetDeletedSessionByVisitorKeyAsync(
        string visitorKey,
        CancellationToken cancellationToken = default)
    {
    ...
    public async Task<StoreChatSession?> GetSessionByIdAsync(
        Guid sessionId,
        CancellationToken cancellationToken = default)
    {
        return await context.StoreChatSessions.FirstOrDefaultAsync(s => s.Id == sessionId, cancellationToken);
    }

    public async Task<List<StoreChatMessage>> GetHistoryAsync(
        Guid sessionId,
        CancellationToken cancellationToken = default)
    {
    ...
    public async Task<List<StoreChatSessionListItemDto>> GetSessionsForStaffAsync(
        CancellationToken cancellationToken = default)
    {
        ...
        return sessions.Select(
            s => new StoreChatSessionListItemDto
            {
                Id = s.Id,
                Mode = s.Mode,
                ContactName = s.ContactName,
                ContactPhone = s.ContactPhone,
                CustomerName = s.CustomerUserId.HasValue ? nameById.GetValueOrDefault(s.CustomerUserId.Value) : null,
                PreviousSessionId = s.PreviousSessionId,
                AssignedStaffId = s.AssignedStaffId,
                AssignedStaffName =
                    s.AssignedStaffId.HasValue ? nameById.GetValueOrDefault(s.AssignedStaffId.Value) : null,
