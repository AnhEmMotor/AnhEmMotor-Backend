	...
	ISieveProcessor sieveProcessor) : IProductCategoryReadRepository
{
		...
		CancellationToken cancellationToken)
	{
		...
		var allQuery = context.GetQuery<CategoryEntity>(DataFetchMode.ActiveOnly)
			.GroupJoin(
				context.ProductCategoryTranslations.Where(tr => tr.LanguageCode == "vi" && tr.DeletedAt == null),
				c => c.Id, tr => tr.ProductCategoryId,
				(c, tvi) => new { c, tvi })
			.GroupJoin(
				context.ProductCategoryTranslations.Where(tr => tr.LanguageCode == "en" && tr.DeletedAt == null),
				x => x.c.Id, tr => tr.ProductCategoryId,
				(x, ten) => new { x.c, tVi = x.tvi, tEn = ten })
			.SelectMany(
				x => x.tEn.DefaultIfEmpty(),
				(x, tEn) => new { x.c, tVi = x.tVi, tEn });
